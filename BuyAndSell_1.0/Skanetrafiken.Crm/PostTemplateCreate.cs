...
{
    public class PostTemplateCreate : Plugin
    {
        /// <summary>
        /// </summary>
        public PostTemplateCreate()
            : base(typeof(PostTemplateCreate))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>(40, "Create", TemplateEntity.EntityLogicalName, new Action<LocalPluginContext>(HandleExecute)));
        ...
        protected void HandleExecute(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                TemplateEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<TemplateEntity>();

                EmailTemplateProxyEntity emailTemplateProxy = new EmailTemplateProxyEntity(target);

                XrmHelper.Create(localContext, emailTemplateProxy);
            }
        }
    }
}
