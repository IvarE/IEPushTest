...
namespace Skanetrafiken.Crm.Entities
{
    ...
    public class ValueCodeEntity : Generated.ed_ValueCode
    {
        ...
        public void CancelValueCode(Plugin.LocalPluginContext localContext)
        {
            //TODO: Send Call to VoucherService to CancelValueCode


            //Update ValueCode
            ValueCodeEntity updateValueCode = new ValueCodeEntity();
            updateValueCode.Id = this.Id;

            updateValueCode.ed_CanceledOn = DateTime.UtcNow;

            if (localContext.PluginExecutionContext != null && localContext.PluginExecutionContext.InitiatingUserId != null)
            {
                SystemUserEntity currentUser = XrmRetrieveHelper.Retrieve<SystemUserEntity>(localContext, localContext.PluginExecutionContext.InitiatingUserId, new ColumnSet(false));
                if (currentUser != null)
                {
                    updateValueCode.ed_CanceledBy = currentUser.ToEntityReference();
                }
            }
            
            //updateValueCode.ed_CanceledBy = user.ToEntityReference();
