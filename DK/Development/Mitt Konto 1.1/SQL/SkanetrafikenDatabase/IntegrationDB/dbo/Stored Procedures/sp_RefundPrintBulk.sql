...
-- =============================================
CREATE PROCEDURE [dbo].[sp_RefundPrintBulk]
...
BEGIN
   ...
   --SELECT a.cgi_caseid
   --FROM  DKCRM_MSCRM.dbo.Filteredcgi_refund a
   ...
   INTO #test1
   FROM  DKCRM_MSCRM.dbo.FilteredSystemUser
   
   SELECT        a.cgi_refundid, d .cgi_printtext
   INTO #test2
   FROM  DKCRM_MSCRM.dbo.Filteredcgi_refund a 
	INNER JOIN  DKCRM_MSCRM.dbo.Filteredcgi_cgi_refundtype_cgi_reimbursementform b ON a.cgi_refundtypeid = b.cgi_refundtypeid 
	INNER JOIN  DKCRM_MSCRM.dbo.Filteredcgi_reimbursementform c ON b.cgi_reimbursementformid = c.cgi_reimbursementformid 
	INNER JOIN DKCRM_MSCRM.dbo.Filteredcgi_refundtype d ON b.cgi_refundtypeid = d .cgi_refundtypeid
...
FROM DKCRM_MSCRM.dbo.FilteredIncident 
INNER JOIN  DKCRM_MSCRM.dbo.Filteredcgi_refund ON Filteredcgi_refund.cgi_caseid = FilteredIncident.incidentid 
LEFT OUTER JOIN  DKCRM_MSCRM.dbo.FilteredAccount ON FilteredIncident.accountid = FilteredAccount.accountid 
LEFT OUTER JOIN  DKCRM_MSCRM.dbo.FilteredContact ON FilteredIncident.contactid = FilteredContact.contactid 
LEFT OUTER JOIN  DKCRM_MSCRM.dbo.Filteredcgi_representative ON FilteredIncident.cgi_representativid = Filteredcgi_representative.cgi_representativeid 
LEFT OUTER JOIN  DKCRM_MSCRM.dbo.FilteredSystemUser ON FilteredIncident.ownerid = FilteredSystemUser.systemuserid 
LEFT OUTER JOIN #test1 ON FilteredIncident.ownerid = #test1.systemuserid 
LEFT OUTER JOIN  DKCRM_MSCRM.dbo.Filteredcgi_amounttranslation ON Filteredcgi_refund.cgi_amount = Filteredcgi_amounttranslation.cgi_amount 
