...
namespace CGIXrmCreateCaseService.Case
{
    public class CreateCaseManager
    {
        ...
        private void CreateLogFile(CreateCaseRequest request)
        {
            ...
            {
                if (request.FileLinks.Count > 0)
                {
                    foreach (var fileLink in request.FileLinks)
                    {
                        if (fileLink.Url != null && fileLink.Url != "")
                            sw.WriteLine("fileLink: " + fileLink.Url);
                        else
                        {
                            if (fileLink.Url == null)
                                sw.WriteLine("fileLink is null");
                            else
                                sw.WriteLine("fileLink is empty");
                        }
                    }
                }
            }
            else
            {
                ...
                {
                    sw.WriteLine("FileLinksList is null");
                }
                else
                {
                    sw.WriteLine("FileLinksList is empty");
                }
            }
