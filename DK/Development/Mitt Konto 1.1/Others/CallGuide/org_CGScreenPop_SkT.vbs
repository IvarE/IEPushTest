...
Const baseURL = "http://server/callguide/createaction.asp" ' base addres for CRM posting '
Const guidURL = "http://server/callguide/getuser.asp" ' base address to retrieve Agent GUID '
...
Const showDebugMessages = true ' if the script shall show all debug info as message boxes, set to true '
...
function ScreenPop()
    ...
    if ContactSourceType = "ivr" or ContactSourceType = "chat" or ContactSourceType = "emailserver" then
        if EventType = "afterNormalAccept" or EventType = "endOfContact" or EventType = "manual" then
...
Function buildURL(agentGUID)
    ...
    result = result & "&chatcustomeralias=" & URLEncode(Params.ParamByName("chatCustomerAlias"))
    result = result & "&agentname=" & URLEncode(Params.ParamByName("agentName"))
