namespace INTSTDK005.STIP.Transformations {
    ...
    public sealed class CreditSafe_GetDataBySecureResponse_To_GetDataBySecureResponse : global::Microsoft.XLANGs.BaseTypes.TransformBase {
              ...
              <xsl:for-each select=""s1:Parameters"">
                <xsl:for-each select=""s0:diffgram/NewDataSet/GETDATA_RESPONSE"">
                  <GETDATA_RESPONSE>
                    ...
                    <xsl:if test=""ORGNR"">
                      <ORGNR>
                        <xsl:value-of select=""ORGNR/text()"" />
                      </ORGNR>
                    </xsl:if>
                    <xsl:if test=""NAME"">
                      <NAME>
                    ...
                    <xsl:if test=""COMPANY_TYPE"">
                      <COMPANY_TYPE>
                        <xsl:value-of select=""COMPANY_TYPE/text()"" />
                      </COMPANY_TYPE>
                    </xsl:if>
                    <xsl:if test=""COMPANY_TYPE_TEXT"">
                      <COMPANY_TYPE_TEXT>
                        <xsl:value-of select=""COMPANY_TYPE_TEXT/text()"" />
                      </COMPANY_TYPE_TEXT>
                    </xsl:if>
                    <xsl:if test=""COMPANY_LEGAL_CODE"">
                      <COMPANY_LEGAL_CODE>
                        <xsl:value-of select=""COMPANY_LEGAL_CODE/text()"" />
                      </COMPANY_LEGAL_CODE>
                    </xsl:if>
                    <xsl:if test=""OLD_NAME"">
                      <OLD_NAME>
                        <xsl:value-of select=""OLD_NAME/text()"" />
                      </OLD_NAME>
                    </xsl:if>
                    ...
                    <xsl:if test=""FAXNR"">
                      <FAXNR>
                        <xsl:value-of select=""FAXNR/text()"" />
                      </FAXNR>
                    </xsl:if>
                    <xsl:if test=""EMAIL_ADRESS"">
                      <EMAIL_ADRESS>
                        <xsl:value-of select=""EMAIL_ADRESS/text()"" />
                      </EMAIL_ADRESS>
                    </xsl:if>
                    <xsl:if test=""WWW_ADRESS"">
                      <WWW_ADRESS>
                        <xsl:value-of select=""WWW_ADRESS/text()"" />
                      </WWW_ADRESS>
                    </xsl:if>
                    <xsl:if test=""INCORPORATION_DATE"">
                      <INCORPORATION_DATE>
                        <xsl:value-of select=""INCORPORATION_DATE/text()"" />
                      </INCORPORATION_DATE>
                    </xsl:if>
                    <xsl:if test=""CFAR_NR"">
                      <CFAR_NR>
                        <xsl:value-of select=""CFAR_NR/text()"" />
                      </CFAR_NR>
                    </xsl:if>
                    <xsl:if test=""COMPANY_STATUS"">
                      <COMPANY_STATUS>
                        <xsl:value-of select=""COMPANY_STATUS/text()"" />
                      </COMPANY_STATUS>
                    </xsl:if>
                    <xsl:if test=""COMPANY_STATUS_DATE"">
                      <COMPANY_STATUS_DATE>
                        <xsl:value-of select=""COMPANY_STATUS_DATE/text()"" />
                      </COMPANY_STATUS_DATE>
                    </xsl:if>
                    <xsl:if test=""COMPANY_ACTIVE"">
                      <COMPANY_ACTIVE>
                        <xsl:value-of select=""COMPANY_ACTIVE/text()"" />
                      </COMPANY_ACTIVE>
                    </xsl:if>
                    <xsl:value-of select=""./text()"" />
                  </GETDATA_RESPONSE>
