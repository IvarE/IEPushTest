...
namespace CGIXrmHandler
{
    public class CallGuideBatchHandler
    {
        ...
        private Entity GetCaseEntityFromWorkitem(WorkItem workitem, CallGuideBatchActivity activiyType)
        {
            ...
            Entity entity = new Entity()
            {
                ...
                Attributes ={
                        ...
                        //new KeyValuePair<string, object>("cgi_casdet_row1_cat1id", e.ErrandTaskType),
                        //new KeyValuePair<string, object>("cgi_casdet_row1_cat2id ", e.ErrandTaskType),
                        //new KeyValuePair<string, object>("cgi_casdet_row1_cat3id ", e.ErrandTaskType),
        ...
        public List<CallGuideRecord> GetCallGuideDBRecord(string interactionIds, CallGuideBatchActivity callguideBatchAction)
        {
            ...
            using (OdbcConnection CallGuideDBCon = new OdbcConnection(connectionString))
            {
                ...
                using (OdbcDataReader reader = command.ExecuteReader())
                {
                    ...
                    Logger.LogMessage("Callguide Record Count =" + lstCallguideRecord.Count, ApplicationName, xrmManager.Service);
        ...
        public List<WorkItem> BulkCreateCase(List<WorkItem> lstToCreatCase, CallGuideBatchActivity activiyType)
        {
            ...
            ExecuteMultipleResponse multipleCreateCaseResponse = (ExecuteMultipleResponse)xrmManager.Service.Execute(multipleCreateCaseRequest);
            ...
            {
                Task.Factory.StartNew(() => Logger.LogMessage("All account records have been updated successfully.", ApplicationName, xrmManager.Service));
        ...
        public List<WorkItem> BulkCreateCaseCategory(List<WorkItem> lstToCreatCaseCategory, CallGuideBatchActivity activiyType)
        {
            ...
            {
                if (xrmHelper.CreateCaseCategory(caseCatergoryToCreate.CreatedCaseId, caseCatergoryToCreate.ErrandTaskType, xrmManager) != Guid.Empty)
            ...
            try
            {
                foreach (WorkItem activityToComplete in lstActivityToComplete)
                {
                    try
                    {
                        ...
                        setStateRequest.EntityMoniker = new EntityReference(activiyType == CallGuideBatchActivity.Chat ? "cgi_callguidechat" : "cgi_callguidefacebook", activityToComplete.ActivityId);


                        setStateRequest.State = new OptionSetValue(1);
                        setStateRequest.Status = new OptionSetValue(2);

                        xrmManager.Service.Execute(setStateRequest);
                    }
                    catch
                    {
                        returnValue = false;
        ...
        public List<WorkItem> BulkUpdateRegarding(List<WorkItem> lstToUpdate, CallGuideBatchActivity activiyType)
        {
            ...
            ExecuteMultipleResponse multipleUpdateActivityResponse = (ExecuteMultipleResponse)xrmManager.Service.Execute(multipleUpdateActivityRequest);
        ...
        public List<WorkItem> GetOpenCrmChatActivity()
        {
            ...
            EntityCollection entityChatActivityCollection = xrmManager.Service.RetrieveMultiple(queryExpChatActivity);
        ...
        public List<WorkItem> GetOpenCrmFaceBookActivity()
        {
            ...
            EntityCollection entityFbActivityCollection = xrmManager.Service.RetrieveMultiple(queryExpFBActivity);
