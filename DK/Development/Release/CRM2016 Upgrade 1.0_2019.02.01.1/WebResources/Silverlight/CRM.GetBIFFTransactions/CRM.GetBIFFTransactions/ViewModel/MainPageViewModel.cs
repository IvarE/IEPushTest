    ...
    public class MainPageViewModel : XrmBaseNotify
    {
        ...
        public MainPageViewModel(MainPage mainpage, CrmManager crmmanager, Dispatcher theDispatcher, string userlcid, string entityid, string data, string debug, string debugcard)
        {
            try
            {
                ...
                _mainpage.btnSearchCardTransactions.IsEnabled = true;
                ToolTipService.SetToolTip(_mainpage.btnSearchCardTransactionsBorder, "");
                ...
                //_mainpage.btnSearchCardTransactions.IsEnabled = false; //Added by Henning Freudenthaler 2017-03-02
                //ToolTipService.SetToolTip(_mainpage.btnSearchCardTransactionsBorder, "You don't have sufficent permissions to search for card transactions!"); //Added by Henning Freudenthaler 2017-03-02
                _mainpage.btnSearchCardTransactions.IsEnabled = true;
                ToolTipService.SetToolTip(_mainpage.btnSearchCardTransactionsBorder, "");
            }
            catch (Exception ex)
        ...
        //Added by Henning Freudenthaler 2017-03-02
        private void _disableButtons()
        {
            string _fieldSecurityRoleName = "AllowCardTransactionSearch";
            string _userID = _user.SystemUserId.ToString(); //"{93560620-6168-E611-80DD-0050569071BE}";
            _getUserFieldSecurityRoles(_fieldSecurityRoleName, _userID);
        }
        private void _getUserFieldSecurityRoles(string _securityRoleName, string _userID)
        {
            try
            {
                _crmManager.Fetch<setting>(_xmlFieldSecurityRoles(_securityRoleName, _userID), _getUserFieldSecurityRoles_completed);
            }
            catch (Exception ex)
            {
                _setLoadingFalse();
                _dispatcher.BeginInvoke(() => { IsWaiting = false; });
                _mainpage.ShowError(ex, "_getUserSecurityRoles");
            }
        }

        private void _getUserFieldSecurityRoles_completed(ObservableCollection<setting> result)
        {
            try
            { 
                // EDIT: Marcus Stenswed
                // Commented out because it's not atually working
                ...
                //            ToolTipService.SetToolTip(_mainpage.btnSearchCardTransactionsBorder, "You don't have sufficent permissions to search for card transactions!");
                //        });
                //    }
                //}
                //else
                ...
                //    _setLoadingFalse();
                //    _dispatcher.BeginInvoke(() => { IsWaiting = false; });
                //    throw new Exception("Fel i hämta user securtiy roles!");
                //}
            }
            catch (Exception ex)
            {
                _setLoadingFalse();
                _dispatcher.BeginInvoke(() => { IsWaiting = false; });
                _mainpage.ShowError(ex, "_getUserSecurityRoles_completed");
            }
        }

        internal static string _xmlFieldSecurityRoles(string _fieldSecurityRoleName, string _userID)
        {
            string _xml = "";

            _xml += "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>";
                _xml += "<entity name='fieldsecurityprofile'>";
                    _xml += "<attribute name='name' />";
                    _xml += "<filter type='and'>";
                        _xml += "<condition attribute='name' operator='eq' value='" + _fieldSecurityRoleName + "' />";
                    _xml += "</filter>";
                    _xml += "<link-entity name='systemuserprofiles' from='fieldsecurityprofileid' to='fieldsecurityprofileid' visible='false' intersect='true'>";
                        _xml += "<link-entity name='systemuser' from='systemuserid' to='systemuserid' alias='af'>";
                            _xml += "<filter type='and'>";
                                _xml += "<condition attribute='systemuserid' operator='eq' uitype='systemuser' value='" + _userID + "' />";
                            _xml += "</filter>";
                        _xml += "</link-entity>";
                    _xml += "</link-entity>";
                _xml += "</entity>";
        ...
        //END - Henning Freudenthaler 2017-03-02
        ...
        private void _initTravelCardService()
        {
            try
            {
                ...
                // Disable search button depending on user permissions - Henning Freudenthaler 2017-03-02
                // Functionality from this method seems like not working - Marcus Stenswed 2018-01-22
                //_disableButtons();
        ...
        private void _loadTranslations()
        {
            try
            {
                _dispatcher.BeginInvoke(() => {
					...
					_mainpage.lblLatestFailedAttempt.Text = _getLocalizedLabel("LABEL_LatestFailedAttempt");
