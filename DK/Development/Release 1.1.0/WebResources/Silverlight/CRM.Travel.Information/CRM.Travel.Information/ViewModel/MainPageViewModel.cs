  ...
  */
  ...
  public class MainPageViewModel {
	...
	private string _xmlGetSavedLines(string id) {
      ...
      _xml += "       <attribute name='cgi_linetype'/>";
	  ...
	  try {
		...
		if (e.Result.DirectJourneysBetweenStops != null) {
		  ...
		  for (int i = 0; i < journeyCount; i++) {
            ...
            _t.LineType = string.IsNullOrEmpty(e.Result.DirectJourneysBetweenStops[i].LineType) ? string.Empty : e.Result.DirectJourneysBetweenStops[i].LineType;
	...
	private void _pubtransClient_GetCallsForServiceJourneyCompleted(object sender, GetCallsForServiceJourneyCompletedEventArgs e) {
	  try {
		...
		*/
		...
		_currentTravelInformation.DisplayText = _setSaveMessage(_currentTravelInformation);
	...
	private void _saveSelectionToCRM(TravelInformation travelinformation) {
	  try {
        ...
        if (!string.IsNullOrEmpty(travelinformation.LineType))
        {
