    ...
    public class CreateCaseManager
    {
        ...
        public Guid GetOrCreateContactRGOLCase(AutoRgCaseRequest request, SettingsEntity settings = null)
        {
            ...
            catch (Microsoft.Rest.HttpOperationException ex)
            {
                ...
                LogMessage(_logLocation, string.Format("Exception catched in GetOrCreateContactRGOLCase:{0}", msg));
        ...
        private RgolSetting TransformEntityToRgolSetting(Entity _customer)
        {
            ...
            if (_customer.Contains("cgi_rgolsettingid"))
                returnValue.RgolSettingId = _customer.GetAttributeValue<EntityReference>("cgi_rgolsettingid").Id;

            if (_customer.Contains("cgi_rgolsettingno"))
                returnValue.RgolSettingNo = _customer.GetAttributeValue<int>("cgi_rgolsettingno").ToString();
            ...
            if (_customer.Contains("cgi_refundtypeid")) {
                returnValue.RefundTypeId = _customer.GetAttributeValue<EntityReference>("cgi_refundtypeid").Id.ToString("D");
                returnValue.RefundTypeIdName = _customer.GetAttributeValue<EntityReference>("cgi_refundtypeid").Name;
            }

            if (_customer.Contains("cgi_reimbursementformid")) {
                returnValue.ReImBursementFormId = _customer.GetAttributeValue<EntityReference>("cgi_reimbursementformid").Id.ToString("D");
                returnValue.ReImBursementFormIdName = _customer.GetAttributeValue<EntityReference>("cgi_reimbursementformid").Name;
            }
        ...
        */
        ...
        internal void RequestCreateCase(CreateCaseRequest request)
        {
            ...
            {
                if (ex.Message != null)
                    LogMessage(@"C:\Temp\createcase.log", string.Format("Exception {0}, inner:{1}", ex.Message, ex.InnerException));
