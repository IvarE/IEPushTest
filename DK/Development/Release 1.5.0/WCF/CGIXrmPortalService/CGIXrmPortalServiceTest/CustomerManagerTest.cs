...
namespace CGIXrmPortalServiceTest
{
    ...
    public class CustomerManagerTest
    {
        ...
        public void CheckCustomerExist()
        {
            ...
            customerEntity = GetCustomerEntity(AccountCategoryCode.Private, Guid.Empty);
            ...
            customerEntity = GetCustomerEntity(AccountCategoryCode.Company, Guid.Empty);
            customerResponse = _customerManager.CheckCustomerExist(customerEntity.Attributes["emailaddress1"].ToString());
            Assert.IsNotNull(customerResponse);
            Assert.AreEqual(customerResponse.Status, ProcessingStatus.SUCCESS);
            Assert.IsNotNull(customerResponse.AccountId);
        ...
        public void CreateCustomer()
        {
            ...
            Customer privateCustomer = getUnitTestCustomer(AccountCategoryCode.Private);
            ...
            //Delete customer
            DeleteCustomerEntity(response.AccountId, privateCustomer.CustomerType);
            #endregion
            ...
            Customer organizationCustomer = getUnitTestCustomer(AccountCategoryCode.Company);
            ...
            //Delete customer
            DeleteCustomerEntity(response.AccountId, organizationCustomer.CustomerType);
            #endregion
        ...
        public void UpdateMyAccountLastLogin()
        {
            ...
            #region UnitTest private customer
            Guid customerId = GetCustomerEntity(AccountCategoryCode.Private, Guid.Empty).Id;
            UpdateCustomerResponse response = _customerManager.UpdateMyAccountLastLogin(customerId, false, myAccountLastLogin);
            Assert.AreEqual(response.Status, ProcessingStatus.SUCCESS);
            Entity customerEntity = GetCustomerEntity(AccountCategoryCode.Private, customerId);
            ...
            customerId = GetCustomerEntity(AccountCategoryCode.Company, Guid.Empty).Id;
            response = _customerManager.UpdateMyAccountLastLogin(customerId, true, myAccountLastLogin);
            Assert.AreEqual(response.Status, ProcessingStatus.SUCCESS);
            customerEntity = GetCustomerEntity(AccountCategoryCode.Company, customerId);
            Assert.IsNotNull(customerEntity);
            Assert.AreEqual(customerEntity.Attributes["cgi_myaccount_lastlogin"], myAccountLastLogin);
            #endregion
        ...
        public void UpdateCustomer()
        {
            ...
            //Create a private customer
            Customer customer = getUnitTestCustomer(AccountCategoryCode.Private);
            ...
            //Delete customer
            DeleteCustomerEntity(response.AccountId, responseCustomer.CustomerType);
            #endregion

            //UnitTest delete private customer
            #region UnitTest delete private customer
            //Create a private customer
            customer = getUnitTestCustomer(AccountCategoryCode.Private);
            //Create the private customer in CRM
            response = _customerManager.CreateCustomer(customer);
            ...
            customer = getUnitTestCustomer(AccountCategoryCode.Company);
            //Create updateCustomer (contains information on what values should be updated)
        ...
        public bool DeleteCustomerEntity(Guid id, AccountCategoryCode type)
        {
            Entity entityId;
        ...
        public Customer getUnitTestCustomer(AccountCategoryCode customerType)
        {
            //Create a private customer
            return new Customer
            {
        ...
        public Entity GetCustomerEntity(AccountCategoryCode type, Guid customerId)
        {
        ...
        public DataCollection<Entity> GetCustomerEntities(AccountCategoryCode type, string emailAddress)
        {
            QueryExpression query;
        ...
        private bool checkCustomerMinimum(Customer customerResponse, Customer customer, AccountCategoryCode type)
        {
            ...
            if (customerResponse.Phone != "") return false;
            if (customerResponse.MobilePhone != "") return false;
        ...
        private bool checkCustomer(Customer customerResponse, Customer customer, AccountCategoryCode type)
        {
            ...
            if (customerResponse.Phone != customer.Phone) return false;
            if (customerResponse.MobilePhone != customer.MobilePhone) return false;
            if (customerResponse.Email != customer.Email) return false;
            if (customerResponse.AllowAutoLoad != customer.AllowAutoLoad) return false;
            if (customerResponse.MaxCardsAutoLoad != customer.MaxCardsAutoLoad) return false;
        ...
        private bool GetCustomerUnitTest(bool fullCustomer, AccountCategoryCode accountCategoryCode)
        {
            ...
            if (fullCustomer)
            {
                customer = getUnitTestCustomer(accountCategoryCode);
            }
            else
            {
        ...
        //Delete customer
            DeleteCustomerEntity(response.AccountId, customerResponse.Customer.CustomerType);
            return true;
        }
