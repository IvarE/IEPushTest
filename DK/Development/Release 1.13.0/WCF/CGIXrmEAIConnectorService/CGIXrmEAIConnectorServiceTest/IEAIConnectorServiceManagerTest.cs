...
namespace CGIXrmEAIConnectorServiceTest
{
    ...
    public class IEAIConnectorServiceManagerTest
    {
        ...
        public void GetTravelCardDetails()
        {
            ...
            //Status: Found
            #region Test2
            travelCards = GetTravelCardNumber(SharedTest.GetMultipleEntityId("cgi_travelcard", new[] { "cgi_travelcardnumber" }, -1));
            login = SharedTest.ValidateLogin(@"D1\CrmAdmin");

            response = _service.GetCustomerIdForTravelCard(
                travelCards,
                login
                );
            Assert.IsNotNull(response);
