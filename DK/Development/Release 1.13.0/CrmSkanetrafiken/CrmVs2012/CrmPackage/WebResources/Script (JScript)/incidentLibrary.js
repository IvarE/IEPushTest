...
CGISweden.incident =
{
    tabOrderFix: function (){
        // Set focus on description
        var descriptionControl = Xrm.Page.ui.controls.get("description");
        descriptionControl.setFocus();

        //// reset tab order
        //var all = document.getElementsByTagName("*");
        ////var dontFocusStart = 5000;
        //for (var i = 0, max = all.length; i < max; i++) {
        //    // Do something with the element here
        //    //all[i].tabIndex = dontFocusStart + i;
        ...
        for (var i = 0; i < Xrm.Page.ui.controls.getLength() ; i++) {
            ...
            if (element.tabIndex && element.tabIndex != "0") {
                ...
                if (!!element.id) // not null
                {
                    var idString = element.id + "";
                    if (idString.indexOf("statecode") > -1)
                        continue;

                    if (element.id == "footer_statecode")
                        continue;
                }

                if ((' ' + element.className + ' ').indexOf(' ' + "ms-crm-Inline-Locked" + ' ') > -1)
                    continue;

                if (element.hasAttribute("controlmode"))
                    if(element.getAttribute("controlmode") == "deactivated")
                        continue;


                if (element.hasAttribute("data-controlmode"))
                    if (element.getAttribute("data-controlmode") == "locked")
                        continue;

                if (element.hasAttribute("data-controlmode"))
                    if (element.getAttribute("data-controlmode") == "deactivated")
                        continue;
    ...
    onFormLoad: function () {
        ...
        CGISweden.incident.tabOrderFix();
    },
