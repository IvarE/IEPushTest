...
namespace CGI.CRM2013.Skanetrafiken.CaseCategory
{
    public class OnCreate_Pre : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                if (data.Context.InputParameters.Contains("Target") &&
                    data.Context.InputParameters["Target"] is Entity)
                {
                    data.Target =
                        (Entity)data.Context.InputParameters["Target"];
        ...
        private void _setCaseCategoryName(PluginData data)
        {
            try
            {
                ...
                {
                    var cat1 = data.Target.Attributes["cgi_casecategory1id"]
                        as EntityReference;
                    if (cat1 != null)
                    {
                        caption += cat1.Name + " ";
                ...
                {
                    var cat2 = data.Target.Attributes["cgi_casecategory2id"]
                        as EntityReference;
                    if (cat2 != null)
                    {
                        caption += cat2.Name + " ";
                ...
                {
                    var cat3 = data.Target.Attributes["cgi_casecategory3id"]
                        as EntityReference;
                    if (cat3 != null)
                    {
                        caption += cat3.Name + " ";
