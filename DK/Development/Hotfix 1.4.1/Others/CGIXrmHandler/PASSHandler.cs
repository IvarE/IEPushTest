...
namespace CGIXrmHandler
{
    public class PASSHandler : IDisposable
    {
        ...
        private EntityReference CreateIncident(CGIXrmHandler.CrmClasses.Incident incident)
        {
            incident.CaseOrigin = new Microsoft.Xrm.Sdk.OptionSetValue((int)CaseOrgin.PhoneCall);
            Entity entity = xrmManager.Create<CGIXrmHandler.CrmClasses.Incident>(incident);
        ...
        private Guid _createContact(Incident incident)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(incident.sPA))
                {
                    ...
                    if (_sPA.Contains(" "))
                    {
                        string[] _split = Regex.Split(_sPA, " ");
                        if (_split.Count() == 1)
                        {
                    ...
                    {
                        _ent.Attributes.Add("address1_postalcode", _sPA);
        ...
        public Guid ExecutePASSRequest(CGIXrmHandler.CrmClasses.Incident incident, CGIXrmHandler.PASS.CrmClasses.PASSTravelInformation[] passTravelInformations)
        {
        ...
        public ObservableCollection<PASS.CrmClasses.Contact> FetchContacts(CrmClasses.Incident incident)
        {
