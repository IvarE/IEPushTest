...
namespace CGIXrmCreateCaseService.Case
{
    public class CreateCaseManager
    {
        ...
        private const string CGI_TRAVEL_INFORMATION = "cgi_travelinformation";
        private const string CGI_BIFF_TRANSACTION = "cgi_travelcardtransaction";
        ...
        {
            // INFO: följande är enligt Max diagram RGOL_Kundidentifikation. 
            //Lite oklart gällande Account vs contact... är alla sökalternativ i diagram för bägge typer?
            Guid customerIdGuid;
            bool isContactOrAccountInCrm;
            Account account = null;
            Account contact = null;

            // guard rule on customerid
            if (Guid.TryParse(request.CustomerId, out customerIdGuid) == false)
            {
                throw new Exception($"Unable to parse the CustomerId from : {request.CustomerId}");
            }

            var isCustomerLoggedInMyAccount = customerIdGuid != Guid.Empty;
            var isOrganisation = request.CustomerType == (int)CustomerType.Organisation;

            // Here we check all different possibilities in a vertical 'if/else if' with minimum nested conditions.
            if (isOrganisation && isCustomerLoggedInMyAccount == false)
            {
            ...
            if (isOrganisation)
            {
                ...
                if (account == null)
                {
                    throw new Exception($"Kunden med id {request.CustomerId} hittas inte i databasen!");
                }

                isContactOrAccountInCrm = true; // anrop till uppdatera metod behövs inte då kund är inloggad och uppgifterna är detsamma
            }
            else if (isCustomerLoggedInMyAccount)
            {
            ...
            {
                isContactOrAccountInCrm = TryGetAccountOrContactByEMail(request.EmailAddress, (CustomerType)request.CustomerType, out contact) ||                                            // finns epost på ärendet [grön ruta i max diagram]
                                          TryGetContactBySocialSecurityNumber(request.SocialSecurityNumber, out contact) ||                                                                  // finns personnummer på ärende [grön ruta max diagram]
                                          TryGetContactByNameAndAddress(request.FirstName, request.LastName, request.Address_Line1, request.Address_PostalCode, request.Address_City, out contact);  // sök efter namn och adress [grön ruta i max diagram]
        ...
        /// <returns></returns>
        private bool TryGetContactBySocialSecurityNumber(string socialSecurityNumber, out Account contactOrAccount)
        {
        ...
        /// <returns></returns>
        private Account GetDefaultCustomer(int? emailCount) //TODO: This private method is never used? Delete (Last check 2016-03-13)
        {
            Account contactOrAccount;
            var crmsetting = GetDefaultCustomer();

            if (crmsetting != null)
            {
                contactOrAccount = new Account
                {
                    AccountId = new Guid(crmsetting.DefaultCustomer),
        ...
        private Guid CreateContact(AutoRgCaseRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(request.FirstName))
                contactEntity.Attributes.Add("firstname", request.FirstName);

            if (!string.IsNullOrEmpty(request.LastName))
                contactEntity.Attributes.Add("lastname", request.LastName);

            if (!string.IsNullOrEmpty(request.EmailAddress))
                contactEntity.Attributes.Add("emailaddress1", request.EmailAddress);

            if (!string.IsNullOrEmpty(request.MobileNo))
                contactEntity.Attributes.Add("telephone2", request.MobileNo);

            if (!string.IsNullOrEmpty(request.Address_Line1))
                contactEntity.Attributes.Add("address1_line1", request.Address_Line1);

            if (!string.IsNullOrEmpty(request.Address_PostalCode))
                contactEntity.Attributes.Add("address1_postalcode", request.Address_PostalCode);

            if (!string.IsNullOrEmpty(request.Address_City))
                contactEntity.Attributes.Add("address1_city", request.Address_City);
        ...
        private void UpdateContactFieldsIfOriginalFieldsAreNull(AutoRgCaseRequest request, Guid accountId)
        {
            if (request.CustomerType != 0) // 0   Privatkund
            {
                return;
            ...
            //hämta nuvarande information om kontakten för att säkerställa att ingen information skrivs över
            var origContact = _xrmManager.Get(
                "contact", 
                accountId, 
                "firstname", 
                "lastname", 
            ...
            if (origContact.Attributes.ContainsKey("firstname") == false && !string.IsNullOrEmpty(request.FirstName))
                origContact.Attributes.Add("firstname", request.FirstName);

            if (origContact.Attributes.ContainsKey("lastname") == false && !string.IsNullOrEmpty(request.LastName))
                origContact.Attributes.Add("lastname", request.LastName);

            if (origContact.Attributes.ContainsKey("emailaddress1") == false && !string.IsNullOrEmpty(request.EmailAddress))
                origContact.Attributes.Add("emailaddress1", request.EmailAddress);

            if (origContact.Attributes.ContainsKey("telephone2") == false && !string.IsNullOrEmpty(request.MobileNo))
                origContact.Attributes.Add("telephone2", request.MobileNo);

            if (origContact.Attributes.ContainsKey("cgi_socialsecuritynumber") == false && !string.IsNullOrEmpty(request.SocialSecurityNumber))
                origContact.Attributes.Add("cgi_socialsecuritynumber", request.SocialSecurityNumber);

            if (origContact.Attributes.ContainsKey("address1_line1") == false && !string.IsNullOrEmpty(request.Address_Line1))
                origContact.Attributes.Add("address1_line1", request.Address_Line1);

            if (origContact.Attributes.ContainsKey("address1_line2") == false && !string.IsNullOrEmpty(request.Address_Line2))
                origContact.Attributes.Add("address1_line2", request.Address_Line2);

            if (origContact.Attributes.ContainsKey("address1_city") == false && !string.IsNullOrEmpty(request.Address_City))
                origContact.Attributes.Add("address1_city", request.Address_City);

            if (origContact.Attributes.ContainsKey("address1_postalcode") == false && !string.IsNullOrEmpty(request.Address_PostalCode))
                origContact.Attributes.Add("address1_postalcode", request.Address_PostalCode);

            if (origContact.Attributes.ContainsKey("address1_country") == false && !string.IsNullOrEmpty(request.Address_Country))
                origContact.Attributes.Add("address1_country", request.Address_Country);
            ...
            out Account account)
        {
            ...
            if (!string.IsNullOrEmpty(addressLine1)) return false;
        ...
        /// <returns></returns>
        private RgolSetting GetRgolSettingFromNo(int refundType)
        {
        ...
        private Guid CreateIncidentRgol(AutoRgCaseRequest request, Account customer)
        {
            var incident = new Entity("Incident")
            {
                Attributes = new AttributeCollection
                {
                    {"caseorigincode", new OptionSetValue(285050007)},
                    {"casetypecode", new OptionSetValue(285050003)},
            ...
            if (!string.IsNullOrEmpty(request.CardNumber))
            {
                incident.Attributes.Add("cgi_travelcardno", request.CardNumber);
            }
            ...
            if (!string.IsNullOrEmpty(request.MobileNo))
            {
                incident.Attributes.Add("cgi_telephonenumber", request.MobileNo);
            }
            ...
            incident.Attributes.Add("cgi_taxiclaimedamount", request.TaxiClaimedAmount);

            var g = _xrmManager.Create(incident);
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        private Guid CreateCustomerRgol(AutoRgCaseRequest request)  //TODO: This private method is never used? Delete (Last check 2016-03-13)
        {
            var customerEntity = new Entity("contact")
            {
                Attributes = new AttributeCollection()
        ...
        internal AutoRgCaseResponse RequestCreateAutoRgCase(AutoRgCaseRequest request)
        {
            ...
            try
            {
                ...
                if (TryIdentifyAccountOrContactFromAutoRg(request, out accountOrContact))
                {
                    // uppdatera privat kund med tillgänliga uppgifter [grön box i max diagram]
                    UpdateContactFieldsIfOriginalFieldsAreNull(request, accountOrContact.AccountId);
                }
                else
                {
                    ...
                    // These values are used in CreateIncidentRgol: TODO check for refac possibility
                    accountOrContact.AccountId = contactId;
                    accountOrContact.AccountName = string.Empty;
                    accountOrContact.RecordCount = 1;
                }
        ...
        internal UpdateAutoRgCaseResponse RequestCreateAutoRgRefundDecision(UpdateAutoRgCaseRequest request)
        {
            ...
            try
            {
                ...
                var incidentCase = GetIncidentFromId(request.CaseID);

                if (!request.Approved)
                    request.RefundType = 0; //ska ge avslag

                //create decision from request
                var setting = GetRgolSettingFromNo(request.RefundType);

                var currency = GetCurrency();

                var refundId = CreateDecisionRgol(request.Value, setting, incidentCase, currency.BaseCurrencyId, request.InternalMessage, request.CustomerMessage);

                if (request.IsCompleted == false && request.Approved == false && request.ReqReceipt)
                {
                ...
                else if (request.IsCompleted == false && request.Approved == false && request.ReqReceipt == false)
                {
                    // Incident status  1    In Progress | 3    Waiting for details
                ...
                {
                    SetRefundDescisionStatus(refundId, 0, 285050005);//285050005 = Approved
                    var rh = new ReimbursementHandler();
                    rh.ExecuteRefundAndUpdatesStatus(refundId, _xrmManager.Service);
                    //leave Incident open
                    // TODO enligt dokument skall det vara stängt här?
                }
                else
                {
                    SetRefundDescisionStatus(refundId, 0, 285050006);//0 = Active, Declined = 285 050 006
        ...
        internal UpdateAutoRgResponse RequestCreateBiffTransactionsLinkedToCase(UpdateAutoRgCaseBiffTransactionsRequest request)
        {
            ...
            try
            {
                ...
                DeleteCurrentEntitiesAttachedToCase(CGI_BIFF_TRANSACTION, request.CaseID, guardErrorMessage);

                foreach (var biffTrans in request.BiffTransactions)
                {
                    var biffTransEntity = new Entity(CGI_BIFF_TRANSACTION)
                    {
                        Attributes = new AttributeCollection
                        {
        ...
        internal UpdateAutoRgResponse RequestCreateTravelInformationLinkedToCase(UpdateAutoRgCaseTravelInformationsRequest request)
        {
            ...
            try
            {
                ...
                DeleteCurrentEntitiesAttachedToCase(CGI_TRAVEL_INFORMATION, request.CaseID, guardErrorMessage);

                foreach (var travelInfo in request.TravelInformations)
                {
                    var travelInformation = new Entity(CGI_TRAVEL_INFORMATION)
                    {
                        Attributes = new AttributeCollection
                        {
                            {"cgi_caseid", new EntityReference("Incident", request.CaseID)}
