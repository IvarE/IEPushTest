    ...
    public class CustomerManager
    {
        ...
        internal CheckCustomerExistResponse CheckCustomerExist(string customerEmail)
        {
            ...
            try
            {
                ...
                Guid accountGuid = xrmHelper.GetIdByValue(customerEmail, "emailaddress1", "contact", xrmMgr, queryActive);
                ...
                {
                    accountGuid = xrmHelper.GetIdByValue(customerEmail, "emailaddress1", "account", xrmMgr, queryActive);
        ...
        public UpdateCustomerResponse UpdateMyAccountLastLogin(Guid customerId, bool isCompany, DateTime myAccountLastLogin)
        {
            ...
            try
            {
                ...
                entity["cgi_myaccount_lastlogin"] = myAccountLastLogin;
                xrmMgr.Service.Update(entity);
                response.Status = ProcessingStatus.SUCCESS;
            }
            catch (FaultException<Microsoft.Xrm.Sdk.OrganizationServiceFault> ex)
            {
        ...
        private static void SetGetCustomerMethodDefaultValueForHasEpiserverAccount(Customer customer)
        {
        ...
        private void HandleFaultException(UpdateCustomerResponse response, FaultException<OrganizationServiceFault> ex, string methodName)
        {
            ...
            string ExceptionMsg = "The application terminated with an error. Timestamp: " + ex.Detail.Timestamp +
                                 ...
                                 "Trace: " + ex.Detail.TraceText +
                                 "Inner Fault: " + (null == ex.Detail.InnerFault ? "No Inner Fault" : "Has Inner Fault");
            log2Crm.Exception(ExceptionMsg, methodName, ex, "Portal Web Service");
            response.Status = ProcessingStatus.FAILED;
            response.Message = ExceptionMsg;
        }
        ...
        private void HandleException(UpdateCustomerResponse response, Exception ex, string methodName)
        {
            ...
            {
                string ExceptionMsg = "The application terminated with an error." + ex.Message;
                log2Crm.Exception(ExceptionMsg, "UpdateMyAccountLastLogin", ex, "Portal Web Service");
