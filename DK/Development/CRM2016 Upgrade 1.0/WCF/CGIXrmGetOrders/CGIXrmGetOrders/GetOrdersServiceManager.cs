...
namespace CGIXrmGetOrders
{
    class GetOrdersServiceManager
    {
        ...
        /// <returns></returns>
        public SavedCreditOrderRowsResponse GetSavedCreditOrderRows(string accountid, string contactid)
        {
            ...
            if (useFetchXml)
            {
                ...
                {
                    #region FetchXML Account

                    _fx += "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>";
                        _fx += "<entity name='cgi_creditorderrow'>";
                            _fx += "<all-attributes />";
                            _fx += "<filter type='and'>";
                                _fx += "<filter type='or'>";
                                    _fx += "<condition attribute='cgi_accountid' operator='eq' uitype='account' value='{" + accountid + "}' />";
                                    //_fx += "<condition attribute='cgi_contactid' operator='eq' uitype='contact' value='{4F02DBF6-CC0D-E511-80D8-005056903A38}' />";
                                _fx += "</filter>";
                            _fx += "</filter>";
                        _fx += "</entity>";
                    _fx += "</fetch>";
                ...
                else if (string.IsNullOrEmpty(contactid) == false)
                {
                    #region FetchXML Contact
                    ...
                    _fx += "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>";
                        _fx += "<entity name='cgi_creditorderrow'>";
                            ...
                            _fx += "<filter type='and'>";
                                _fx += "<filter type='or'>";
                                    //_fx += "<condition attribute='cgi_accountid' operator='eq' uitype='account' value='{D4793743-8DEA-E411-80D8-005056903A38}' />";
                                    _fx += "<condition attribute='cgi_contactid' operator='eq' uitype='contact' value='{" + contactid + "}' />";
                                _fx += "</filter>";
                            _fx += "</filter>";
                        _fx += "</entity>";
                    _fx += "</fetch>";
                ...
                EntityCollection creditOrderRows = _crmmanager.Service.RetrieveMultiple(new FetchExpression(_fx));

                //return settings.Entities.First();
                response.OrderList = TransformEntityCollectionToListOfCreditOrderRow(creditOrderRows, isCustomerAccount);
                response.ErrorMessage = "";
            }
            else
            {

                #region old sp_GetCreditOrderRowsForAccount & sp_GetCreditOrderRowsForContact
                try
                {
        ...
        /// <param name="isCustomerAccount"></param>
        /// <returns></returns>
        private List<CreditOrderRow> TransformEntityCollectionToListOfCreditOrderRow(EntityCollection _customers, bool isCustomerAccount)
        {
            List<CreditOrderRow> returnValues = new List<CreditOrderRow>();

            foreach (Entity _customer in _customers.Entities) {

                CreditOrderRow returnValue = new CreditOrderRow();

                if (isCustomerAccount)
                {
                    returnValue.Accountid = _customer.Id.ToString("D");
                }
                else
                {
                    returnValue.Contactid = _customer.Id.ToString("D");
                }

                if (_customer.Contains("cgi_date"))
                    returnValue.Date = _customer.GetAttributeValue<string>("cgi_date");

                if (_customer.Contains("cgi_name"))
                    returnValue.Name = _customer.GetAttributeValue<string>("cgi_name");

                if (_customer.Contains("cgi_ordernumber"))
                    returnValue.OrderNumber = _customer.GetAttributeValue<string>("cgi_ordernumber");

                if (_customer.Contains("cgi_referencenumber"))
                    returnValue.ReferenceNumber = _customer.GetAttributeValue<string>("cgi_referencenumber");

                if (_customer.Contains("cgi_success"))
                    returnValue.Success = _customer.GetAttributeValue<string>("cgi_success");

                if (_customer.Contains("cgi_sum"))
                    returnValue.Sum = _customer.GetAttributeValue<string>("cgi_sum");

                if (_customer.Contains("cgi_time"))
                    returnValue.Time = _customer.GetAttributeValue<string>("cgi_time");

                if (_customer.Contains("cgi_productnumber"))
                    returnValue.Productnumber = _customer.GetAttributeValue<string>("cgi_productnumber");

                if (_customer.Contains("cgi_reason"))
                    returnValue.Reason = _customer.GetAttributeValue<string>("cgi_reason");

                if (_customer.Contains("cgi_createdby"))
                    returnValue.CreatedBy = _customer.GetAttributeValue<string>("cgi_createdby");
                ...
                if (!string.IsNullOrEmpty(xmlresponse))
                {
                    ...
                    foreach (var orderElement in ordersElement)
                    {
                        ...
                        foreach (var orderItemChildElement in orderItemParentElement)
                        {
                            ...
                            string PacketId = GetValueFromXML(orderItemChildElement, "PacketId");
        ...
        {
            //Remove to test that we dont have any functions that uses integrationDB
            /*
            lock (_lockSql)
            {
