...
namespace CGIXrmRainDanceExport
{
    public class RunBatch_RainDance2_Sverige
    {
        ...
        private string CreateCustomerRecord(RefundEntity refund, ContactEntity contact, IncidentEntity incident)
        {
            ...
            string momskod = FormatVatCode(refund.cgi_vat_code); // "1500".SetToFixedLengthPadRight(16);
            ...
            if (!string.IsNullOrWhiteSpace(incident.cgi_rgol_fullname))//if (incident.Caseorigincode == 285050007)
            {
                ...
                postnummer = $"{_formatString(FormatPostnummer(incident.cgi_rgol_address1_postalcode))}".SetMaxLength(30);
                ort = _formatString(incident.cgi_rgol_address1_city).SetToFixedLengthPadRight(30);
            }

            string line = string.Format($"{postmarkering}{kundidentitet}{kundnamn}{kundco}{postadress}{postnummer}{ort}{momskod}{personnummer}{landskod}{motpart}{kundtyp}{peppoId}{plusgiro}{bankgiro}{bankkonto}{gln}{iban}{bicswift}{riksbankskode}");
            return line;
        }
        ...
        private string CreateInvoiceHeader(RefundEntity refund, ContactEntity contact)
        {
            ...
            string line = string.Format($"{postmarkering}{kundidentitet}{fakturadatum}{forfallodatum}{bokforingsnamn}{erreferens}{fakturanummer}{varreferens}{fakturaavser}{motpart}{personnummer}{reskontraposttyp}{ikpart}");
            _countInvoince++;
        ...
        private string CreateInvoiceRow(RefundEntity refund, IncidentEntity incident)
        {
            ...
            if (vatCode == int.MinValue)
            {
                belopp = _calculatenetamount(refund.cgi_Amount, null).SetToFixedLengthPadRight(16); //ex moms
                momskod = _formatVatCode(null).SetToFixedLengthPadRight(2);
            }
            else
            {
                string vatName = getlabelFromValueOptionSet(vatCode);
                belopp = _calculatenetamount(refund.cgi_Amount, vatName).SetToFixedLengthPadRight(16); //ex moms
                momskod = _formatVatCode(vatName).SetToFixedLengthPadRight(2);
            }
            string kreditmarkering = "-".SetToFixedLengthPadRight(1);
            string line = string.Format($"{postmarkering}{betalningsmeddelande}{belopp}{kreditmarkering}{momskod}");
            return line;
        ...
        private string CreateAccountingRecord(RefundEntity refund, ContactEntity contact, IncidentEntity incident, RefundResponsibleEntity responsible, RefundProductEntity refundproduct, UserEntity user, RefundAccountEntity refundaccount)
        {
            ...
            string line = string.Format($"{postmarkering}{konto}{ansvar}{proj}{aktiv}{objekt}{motpart}{momscode}{belopp}{kreditmarkering}{bokforingstext}{periodiseringsnyckel}{periodiseringfrom}{periodiseringto}{attestSAK}{attestBES}");
            return line;
        }
