    ...
    public class RunBatch_Kundfaktura
    {
        ...
        public bool Run()
        {
            ...
            try
            {
                ...
                _fileName = string.Format("{0}\\DK_Kundfaktura_{1}", _exportdir, _now + ".txt");
                ...
                Console.WriteLine("Number of refunds to process: " + _refunds.Count);
                foreach (refund _refund in _refunds)
                {
                    Console.WriteLine(string.Format("Refundnr: {0}; amount: {1}; vat code name: {2}; net amount: {3}", _refund.Refundnumber, _refund.Amount.Value.ToString("#,0.00", CultureInfo.CreateSpecificCulture("sv-SE")), _refund.Vat_code_name, _calculatenetamount(_refund.Amount, _refund.Vat_code_name)));
                    try
                    {
                        //Console.WriteLine(_formatString(_refund.Caseid.ToString()));
                        ...
                        if (_refund.Caseid != null)
                        {
                            ...
                            invoicerecipient _invoicerecipient = _getCurrentInvoiceRecipient(_refund.Invoicerecipient.ToString());
                            if (_contact != null)
                            {
                                if (_incident != null)
                                {
                                    if (_invoicerecipient != null)
                                    {
                                        //string _custinfo = _createCustomerRecord(_refund, _contact);
                                        string _inviceinfo = _createInvoiceRecord(_refund, _invoicerecipient, _incident);
                                        IEnumerable<string> _inforecords = _createInformationRecord(_refund, _incident);
                                        string _invoicerow = _createInvoiceRowRecord(_refund, _incident);
                                        ...
                                        foreach (string _inforecord in _inforecords)
                                        {
                                            _exportdata = new ExportData();
                                            _exportdata.Counter = _count;
                                    ...
                                    {
                                        _logErrorOnRefund(_refund.Refundid, "Ingen fakturamottagare hittas på ersättningsposten.");
                                    }
                                }
                                else
                                {
                                    _logErrorOnRefund(_refund.Refundid, "Ingen ärendekoppling hittas på ersättningsposten.");
                                }
                            }
                            else
                            {
        ...
        //{
        //    string _line1 = "02";
        //    string _line2 = _formatSocNumber(_formatString(refund.Soc_sec_number), _formatString(refund.Foreign_payment)).SetToFixedLength(15);
        //    string _line3 = string.Format("{0} {1}", _formatString(contact.Lastname), _formatString(contact.Firstname)).SetToFixedLength(30);
        //    string _line4 = contact.Address1_line1.SetToFixedLength(30);
        //    string _line5 = string.Format("{0}  {1}", _formatString(contact.Address1_postalcode), _formatString(contact.Address1_city)).SetToFixedLength(30);
        //    string _line6 = "1500".SetToFixedLength(12);
        //    string _line7 = "".PadLeft(16);         // "99".PadLeft(16, '0');
        //    string _line = string.Format("{0}{1}{2}{3}{4}{5}{6}", _line1, _line2, _line3, _line4, _line5, _line6, _line7);
        //    return _line;
        //}
        ...
        private string _createInvoiceRecord(refund refund, invoicerecipient _invoicerecipient, incident incident)
        {
            ...
            string _line1 = "03";
            string _line2 = _formatString(_invoicerecipient.cgi_customer_no).SetToFixedLengthPadRight(15);
            string _line3 = _formatCreateDate(DateTime.Now).SetToFixedLengthPadRight(8);
            string _line4 = "".SetToFixedLengthPadRight(8);
            string _line5 = _formatCreateDate(DateTime.Now).SetToFixedLengthPadRight(8);
            string _line6 = "".SetToFixedLengthPadRight(8);
            string _line7 = _formatString(_invoicerecipient.cgi_inv_reference).SetToFixedLengthPadRight(50);
            string _line8 = string.Format("Handläggare: {0}", incident.CreatedByName).SetToFixedLengthPadRight(50);
            string _line9 = string.Format("Telefonnummer: {0}", reinvoicingphonenumber).SetToFixedLengthPadRight(50);
        ...
        private IEnumerable<string> _createInformationRecord(refund refund, incident incident)
        {
            ...
            //string IncidentDescription = "012345\n678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789";
            ...
            {
                string _line1 = "04";
                string _line2 = s.SetToFixedLengthPadRight(50);
                string _line = string.Format("{0}{1}", _line1, _line2);
                returnValue.Add(_line);
            }
        ...
        private string _createInvoiceRowRecord(refund refund, incident incident)
        {
            ...
            string _line1 = "05";
            string _line2 = string.Format("Ärendenr: {0}", _formatString(incident.Ticketnumber)).SetToFixedLengthPadRight(50);
            string _line3 = _calculatenetamount(refund.Amount, refund.Vat_code_name).SetToFixedLengthPadRight(16); //ex moms
        ...
        private string _createAcountingRecord(refund refund, contact contact, incident incident, responsible responsible, refundproduct refundproduct, user user, refundaccount refundaccount)
        {
            ...
            string _line10 = _calculatenetamount(refund.Amount, refund.Vat_code_name).SetToFixedLengthPadRight(16); //ex moms
        ...
        {
            invoicerecipient _invoicerecipient = null;

            ObservableCollection<invoicerecipient> _invoicerecipients = _xrmManager.Get<invoicerecipient>(_xmlInvoiceRecipient(invoicerecipientid));
            if (_invoicerecipients != null && _invoicerecipients.Count() > 0)
                _invoicerecipient = _invoicerecipients[0];
        ...
        private void _logErrorOnRefund(Guid refundid, string ex)
        {
            ...
            try
            {
                ...
                _refund.Attributes.Add("cgi_invoiceexportmessage", ex);
                _refund.Attributes.Add("cgi_invoiceexportedraindance", false);
                _refund.Attributes.Add("cgi_invoiceexportdate", DateTime.Now);

                _xrmManager.Update(_refund);
            }
            catch (Exception exe)
            {
                EntityReference refund_ref = new EntityReference("cgi_refund", refundid);
                _CreateAnnotation("ExportError", string.Format("Error occured when logging error on refund: {0}", exe.Message), refund_ref);
                _CreateAnnotation("ExportLogMessage", string.Format("{0}", exe), refund_ref);
            }
        }
        ...
        private void _setRecordToExported(Guid refundid)
        {
            ...
            _refund.Attributes.Add("cgi_invoiceexportmessage", "");
            _refund.Attributes.Add("cgi_invoiceexportedraindance", true);
            _refund.Attributes.Add("cgi_invoiceexportdate", DateTime.Now);
        ...
        private string _getxmlPendingRefunds()
        {
            ...
            _xml += "       <attribute name='cgi_invoiceexportedraindance' />";
            _xml += "       <attribute name='cgi_invoicerecipient' />";
            ...
            _xml += "           <condition attribute='cgi_reinvoicing' operator='eq' value='1' />";
            _xml += "           <condition attribute='cgi_amount' operator='gt' value='0' />";
           
            //_xml += "           <condition attribute='cgi_accountid' operator='not-null' />";
            //_xml += "           <condition attribute='cgi_invoicerecipient' operator='not-null' />";

            _xml += "           <filter type='or'>";
            _xml += "               <condition attribute='cgi_invoiceexportedraindance' operator='eq' value='0' />";
            _xml += "               <condition attribute='cgi_invoiceexportedraindance' operator='null' />";
            _xml += "           </filter>";

            //_xml += "           <condition attribute='createdon' operator='on-or-after' value='2015-11-01' />";
            //_xml += "           <condition attribute='createdon' operator='on-or-before' value='2015-09-30' />";
            _xml += "       </filter>";
        ...
        private void GetSettings(IOrganizationService service, out string cgi_reinvoicingphonenumber)
        {
            ...
            _xml += "       <attribute name='cgi_reinvoicingphonenumber' />";
            _xml += "       <filter type='and'>";
            _xml += "           <condition attribute='statecode' operator='eq' value='0' />";
        ...
        private string _xmlInvoiceRecipient(string invoicerecipientid)
        {
            ...
            _xml += "<fetch version='1.0' mapping='logical' distinct='false'>";
            _xml += "    <entity name='cgi_invoicerecipient'>";
            _xml += "        <attribute name='cgi_invoicerecipientid' />";
            _xml += "        <attribute name='cgi_address1' />";
            _xml += "        <attribute name='cgi_customer_no' />";
            _xml += "        <attribute name='cgi_inv_reference' />";
            _xml += "        <attribute name='cgi_invoicerecipientname' />";
            _xml += "        <attribute name='cgi_postal_city' />";
            _xml += "        <attribute name='cgi_postalcode' />";
            _xml += "        <filter type='and'>";
            _xml += "            <condition attribute='cgi_invoicerecipientid' operator='eq' value='" + invoicerecipientid + "' />";
            _xml += "        </filter>";
            _xml += "    </entity>";
            _xml += "</fetch>";
        ...
        private string _xmlCase(string caseid)
        {
            ...
            _xml += "       <attribute name='cgi_contactid' />";
            _xml += "       <attribute name='cgi_accountid' />";
            _xml += "       <attribute name='description' />";
            _xml += "       <attribute name='createdby' />";
            _xml += "       <filter type='and'>";
