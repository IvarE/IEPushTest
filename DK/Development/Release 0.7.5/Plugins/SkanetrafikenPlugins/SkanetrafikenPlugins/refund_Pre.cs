...
namespace CRM2013.SkanetrafikenPlugins
{
    public class refund_Pre : IPlugin
    {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                ...
                if (_data.Context.InputParameters.Contains("Target") && _data.Context.InputParameters["Target"] is Entity)
                {
                    ...
                    {
                        _checkIfContactHasAddress(_data);
                    }
            ...
            {
                throw new Exception(String.Format("InvalidPluginExecutionException message: {0}", ex.InnerException));
            }
            catch (FaultException<OrganizationServiceFault> ex)
            {
                throw new Exception(String.Format("FaultException message: {0}", ex.InnerException));
            }
            catch (Exception ex)
            {
                throw new Exception(String.Format("Exception message: {0}", ex.InnerException));
            }
        }
        ...
        private void _checkIfContactHasAddress(plugindata data)
        {
            try
            {
