    ...
    public class MainPageViewModel : XrmBaseNotify
    {
        ...
        public MainPageViewModel(WebParameters webParameters, CrmManager crmManager, Dispatcher dispatcher, MainPage mainpage, string querystring, string initparams)
        {
            try
            {
                ...
                // TODO: refactor to use the properties that reset the values of either id type
                        ...
                        else if (IsExecutingInStandaloneSearchOrderArea == true /*&& _savedCreditOrderRows == null*/)
                        {
                            ...
                            */
                            ...
                            if (orderHeader != null)
                            {
                                ...
                                //string crmAddressBase = GetCRMBaseAddress();

                                if (isCompany != null)
                                {
                                    //int histKeyRandom = new Random().Next(100000000,999999999);
                                    string customerName = orderHeader.Customer.Name;
                                    if (isCompany == true)
                                    {
                                        //CreateLinkToAccountOrContact(accountNumber, isCompany, crmAddressBase, histKeyRandom);
        ...
        // TODO : remove
        private void CreateLinkToAccountOrContact(string accountNumber, bool? isCompany, string crmAddressBase, int histKeyRandom)
        {
            if (string.IsNullOrEmpty(accountNumber) == false)
            {
                this.LinkToContactOrAccount = GetEntityLink(isCompany.Value, accountNumber, crmAddressBase, histKeyRandom);
                this.LinkName = GetEntityLink(isCompany.Value, accountNumber, crmAddressBase, histKeyRandom);
            }
            else
            {
                this.LinkName = "Länk kan inte skapas.";
            }
        }
        ...
        private void SetName_ToLinkToAccountOrContact(string contactId, string accountId)
        {
            if (string.IsNullOrEmpty(contactId) == false || string.IsNullOrEmpty(accountId) == false)
            {
        ...
        private static string GetEntityLink(bool isAccount, string accountNumber, string crmAddressBase, int histKeyRandom)
        {
            ...
            if (isAccount == true)
            {
                return crmAddressBase + "main.aspx?etc=1&extraqs=&histKey=" + histKeyRandom + "&id=%7B" + accountNumber + "%7D&newWindow=true&pagetype=entityrecord";
            }
            return crmAddressBase + "main.aspx?etc=2&extraqs=&histKey=" + histKeyRandom + "&id=%7B" + accountNumber + "%7D&newWindow=true&pagetype=entityrecord";
             */
        }
        ...
        /*
        private static string GetCRMBaseAddress()
        {
            string pageUri = System.Windows.Browser.HtmlPage.Document.DocumentUri.ToString();
            string[] pageUris = pageUri.Split('/');
            string crmAddressBase = string.Empty;
            for (int i = 0; i < 4; i++)
            {
                string j = pageUris[i];
                crmAddressBase += j + "/";
            }
            return crmAddressBase;
        }
        */
                ...
                if (order_DataTransferObject != null)
                {
                    ...
                    {/*
                        ...
                        */
                        ...
                        //if (IsExecutingInStandaloneSearchOrderArea)
                        //{
                        /* if (notCreditableSum_credits_done != 0)
                         {
                             order_ViewObject.AmountThatCanNotBeCredit += notCreditableSum_credits_done;
                         }*/
                        //}
                    ...
                    // TODO:  not in use? delete. Check xaml manually
