...
namespace CRM2013.SkanetrafikenPlugins
{
    public class refund_update : IPlugin
    {
        ...
        {
            //TODO: Since the change was commited prior branching to Release 0.6.0, this code has been disabled in order to avoid
            //a potential risk in production environment due to a failed test. Code in method body should be re-enabled as part of
            //Release 0.7.0 and re-tested.
        ...
        private void CancelGiftCode(Entity refund, plugindata pluginData)
        {
            ...
            if (refund.IsActive())
            {
                throw new InvalidPluginExecutionException("Cannot cancel a gift code on an active refund");
            }
            
            if (refund.GiftCodeHasBeenUsed() || refund.GiftCodeHasExpired()) //Used or expired codes cannot be canceled
            {
                throw new InvalidPluginExecutionException("Cannot cancel a gift code that has been used or has expired");
            }
        ...
        private Entity CreateRefundWithNoValueCode(Entity oldRefund)
        {
            ...
            newRefund["cgi_value_code_used"] = DateTime.Now;
            return newRefund;
        }
