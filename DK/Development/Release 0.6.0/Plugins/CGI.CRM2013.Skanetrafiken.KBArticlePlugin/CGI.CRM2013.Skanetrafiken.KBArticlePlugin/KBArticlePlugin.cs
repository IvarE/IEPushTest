...
namespace CGI.CRM2013.Skanetrafiken.KBArticlePlugin {
  public class KBArticlePlugin : IPlugin {
	...
	public WCFService.Article GetKBArticleIntranet(Entity preImage) {
	  ...
	  kbArticleIntranet.InternalText = preImage.Attributes.Contains("content") ? preImage.Attributes["content"].ToString() : string.Empty;
	  kbArticleIntranet.ExternalText = preImage.Attributes.Contains("cgi_externaltext") ? preImage.Attributes["cgi_externaltext"].ToString() : string.Empty;
	  kbArticleIntranet.Authors = new string[] { ((EntityReference)preImage.Attributes["createdby"]).Name };
	...
	public WCFService.Article GetKBArticleExternalWeb(Entity preImage) {
	  ...
	  //kbArticleExternalWeb.SearchHitTypeName = GetSearchHitTypeNameString(_pluginConfigXML, "Configuration/SearchHitTypeName");
	  kbArticleExternalWeb.ExternalText = preImage.Attributes.Contains("cgi_externaltext") ? preImage.Attributes["cgi_externaltext"].ToString() : string.Empty;
	  kbArticleExternalWeb.Authors = new string[] { ((EntityReference)preImage.Attributes["createdby"]).Name };
