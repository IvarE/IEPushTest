...
namespace CRM.GetBIFFTransactions.ViewModel
{
    public class MainPageViewModel : XrmBaseNotify
    {
        ...
        private void _travelCardClient_GetCardDetailsCompleted(object sender, GetCardDetailsCompletedEventArgs e)
        {
            try
            {
                if (e.Result != null)
                {
                    ...
                    {
                        string _card = _mainpage.txtTravelCard.Text;
                        _travelCardClient.GetCardFromCRMCompleted += _travelCardClient_GetCardFromCRMCompleted;
                        _travelCardClient.GetCardFromCRMAsync(_card);
                    }
                    else
                    {
                        IsWaiting = false;
        ...
        private void _travelCardClient_GetCardFromCRMCompleted(object sender, GetCardFromCRMCompletedEventArgs e)
        {
            try
            {
                if (e.Result != null)
                {
                    ...
                    {
                        IsWaiting = false;
                        _mainpage.ShowMessage(_response.ErrorMessage, "_travelCardClient_GetCardFromCRMCompleted");
                        return;
                    }
            ...
            {
                IsWaiting = false;
                _mainpage.ShowError(ex, "_travelCardClient_GetCardFromCRMCompleted");
            }
            finally
            {
                _travelCardClient.GetCardFromCRMCompleted -= _travelCardClient_GetCardFromCRMCompleted;
