...
CGISweden.refund =
{
    ...
    getCaseNumber_callback: function (result) {
        try {
            if (result == null || result[0] == null) {
                ...
                //MaxP 2016-03-01 
                _caseorigincode = result[0].CaseOriginCode.Value;
                _casetypecode = result[0].CaseTypeCode.Value;
                _iscompleted = result[0].cgi_iscompleted

                //alert(_caseorigincode);
                //alert(_casetypecode);
                //alert(_iscompleted);
                ...
                if (_caseorigincode == 285050007 && _casetypecode == 285050003 && _iscompleted == false) {
                    ...
                    //Sätt värden beroende på ersättningstyp
                    _milagekilometers = result[0].cgi_milagekilometers;
                    _taxiclaimedamount = result[0].cgi_taxiclaimedamount;

                    if (_milagekilometers != null && _milagekilometers != 0) {
                        var _milagekilometers_decimal = parseFloat(_milagekilometers);
                        Xrm.Page.getAttribute("cgi_milage").setValue(_milagekilometers_decimal);
                        var _cgi_amount = Math.round(_milagekilometers_decimal * 18,50 /10);
                        Xrm.Page.getAttribute("cgi_amount").setValue(_cgi_amount);
                        Xrm.Page.getAttribute("cgi_calculated_amount").setValue(_cgi_amount);
                    }

                    if (_taxiclaimedamount != 0 && _taxiclaimedamount != null) {
    ...
    SetDateValidField: function () {
        try {
            ...
            var futureDate = CGISweden.refund.getFutureDate(180);
    ...
    getGetEmailAddress_callback: function (result) {
        try {
            if (result == null || result[0] == null) {
                ...
                var _cgi_email = result[0].cgi_customer_email;
                CGISweden.formscriptfunctions.SetValue("cgi_email", _cgi_email);
            }
        }
        catch (e) {
