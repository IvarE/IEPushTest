...
namespace Endeavor.Crm.UnitTest
{
    ...
    public class LeadFixture : PluginFixtureBase
    {
        ...
        private static readonly string CampaignCode = "777777";      
        ...
        public void ToLeadInfoTest()
        {
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                {
                    //Skapa en ny produkt
                    ProductEntity product = getTestProduct(localContext);

                    //Skapa en kampanj
                    CampaignEntity newCampaign = CampaignFixture.createNewCampaign(localContext);

                    //Associera produkt till kampanj
                    ...
                    //Skapa ett lead
                    LeadEntity newLead = createNewLead(localContext, newCampaign);
                    ...
                    Assert.AreEqual(newCampaign.CodeName, info.CampaignId);
        ...
        public void PostLeadQualify()
        {
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                try
                {
                    CampaignEntity newCampaign = CampaignFixture.CreateCampaign(localContext);
                    LeadEntity qualLead = new LeadEntity
                    {
                        Id = Guid.NewGuid(),

                    };

                    LeadEntity newLeadOne = createNewLead(localContext, newCampaign);
                    LeadEntity newLeadTwo = createNewLead(localContext, newCampaign);
                    LeadEntity newLeadThree = createNewLead(localContext, newCampaign);

                    //KANSkE TA BORT DENNA
                    IList<LeadEntity> connectedLeads = XrmRetrieveHelper.RetrieveMultiple<LeadEntity>(localContext, new ColumnSet(true),
                    new FilterExpression
                    {
                        Conditions =
                        {
