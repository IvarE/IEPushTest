...
namespace Skanetrafiken.Crm.Entities
{
    public class ContactEntity : Generated.Contact
    {
        ...
        internal void HandlePostContactCreateAsync(Plugin.LocalPluginContext localContext)
        {
            try
            {
                if (Generated.ContactState.Active.Equals(this.StateCode) && !string.IsNullOrWhiteSpace(this.cgi_socialsecuritynumber))
                {
                    DeltabatchQueueEntity queue = new DeltabatchQueueEntity
                    {
                        ...
                        ed_name = $"Create: {this.FullName}, {DateTime.Now.ToString()}"
                    };
            ...
            try
            {
                if (Generated.ContactState.Active.Equals(this.StateCode) && !string.IsNullOrWhiteSpace(this.cgi_socialsecuritynumber))
                {
                    DeltabatchQueueEntity queue = new DeltabatchQueueEntity
                    {
                        ...
                        ed_name = $"Delete!: {this.FullName}, {DateTime.Now.ToString()}"
                    };
                    queue.Id = XrmHelper.Create(localContext.OrganizationService, queue);
                }
            }
        ...
        internal void HandlePostContactUpdateAsync(Plugin.LocalPluginContext localContext, ContactEntity preImage)
        {
            ...
            try
            {
                if (combined.cgi_socialsecuritynumber != preImage.cgi_socialsecuritynumber)
                {
                    if (combined.StateCode.Equals(Generated.ContactState.Active) && !string.IsNullOrWhiteSpace(combined.cgi_socialsecuritynumber))
                    {
                        DeltabatchQueueEntity plusQueue = new DeltabatchQueueEntity
                        {
                            ed_ContactNumber = combined.cgi_socialsecuritynumber,
                            ed_DeltabatchOperation = Generated.ed_deltabatchqueue_ed_deltabatchoperation.Plus,
                            ed_name = $"Update - SocSecNr: {combined.FullName}, {DateTime.Now.ToString()}"
                        };
                        plusQueue.Id = XrmHelper.Create(localContext.OrganizationService, plusQueue);
                    }
                    if (!string.IsNullOrWhiteSpace(preImage.cgi_socialsecuritynumber))
                    {
                        DeltabatchQueueEntity minusQueue = new DeltabatchQueueEntity
                        {
                            ed_ContactNumber = preImage.cgi_socialsecuritynumber,
                            ed_DeltabatchOperation = Generated.ed_deltabatchqueue_ed_deltabatchoperation.Minus,
                            ed_name = $"Update - SocSecNr: {preImage.FullName}, {DateTime.Now.ToString()}"
                        };
        ...
        internal static void HandlePostContactSetStateAsync(Plugin.LocalPluginContext localContext, EntityReference myEntityRef, OptionSetValue state, OptionSetValue status)
        {
            try
            {
                ContactEntity contact = XrmRetrieveHelper.Retrieve<ContactEntity>(localContext, myEntityRef, new ColumnSet(ContactEntity.Fields.StateCode, ContactEntity.Fields.cgi_socialsecuritynumber, ContactEntity.Fields.FullName));

                if (!string.IsNullOrWhiteSpace(contact.cgi_socialsecuritynumber))
                {
                    switch (contact.StateCode)
                    {
                        case Generated.ContactState.Active:
                            DeltabatchQueueEntity plusQueue = new DeltabatchQueueEntity
                            {
                                ...
                                ed_DeltabatchOperation = Generated.ed_deltabatchqueue_ed_deltabatchoperation.Plus,
                                ed_name = $"Update - Activated: {contact.FullName}, {DateTime.Now.ToString()}"
                            };
                            plusQueue.Id = XrmHelper.Create(localContext.OrganizationService, plusQueue);
                            break;
                        case Generated.ContactState.Inactive:
                            DeltabatchQueueEntity minusQueue = new DeltabatchQueueEntity
                            {
                                ed_ContactNumber = contact.cgi_socialsecuritynumber,
                                ed_DeltabatchOperation = Generated.ed_deltabatchqueue_ed_deltabatchoperation.Minus,
                                ed_name = $"Update - Deactivated: {contact.FullName}, {DateTime.Now.ToString()}"
                            };
                            minusQueue.Id = XrmHelper.Create(localContext.OrganizationService, minusQueue);
                            break;
                        default:
        ...
        public CustomerInfo ToContactInfo(Plugin.LocalPluginContext localContext)
        {
        ...
        public void HandlePreContactCreate(Plugin.LocalPluginContext localContext)
        {
            ...
            Address1_Line1 = Capitalise(Address1_Line1);
            Address2_City = Capitalise(Address2_City);
            Address2_Line1 = Capitalise(Address2_Line1);
            ...
            {
                if (Contains(ContactEntity.Fields.EMailAddress1))
        ...
        public void HandlePreContactUpdate(Plugin.LocalPluginContext localContext, ContactEntity preImage)
        {
            ...
            if (Contains(ContactEntity.Fields.Address1_City))
                Address1_City = Capitalise(Address1_City);
            if (Contains(ContactEntity.Fields.Address1_Line1))
                Address1_Line1 = Capitalise(Address1_Line1);
            if (Contains(ContactEntity.Fields.Address2_City))
                Address2_City = Capitalise(Address2_City);
            if (Contains(ContactEntity.Fields.Address2_Line1))
                Address2_Line1 = Capitalise(Address2_Line1);
            if (!string.IsNullOrWhiteSpace(EMailAddress1))
        ...
        public void HandlePostContactUpdate(Plugin.LocalPluginContext localContext, ContactEntity preImage)
        {
            ...
            if (combined.Contains(ContactEntity.Fields.EMailAddress1) && !string.IsNullOrWhiteSpace(combined.EMailAddress1) &&
        ...
        private void VerifyNoDuplicates(Plugin.LocalPluginContext localContext, Generated.ed_informationsource infoSource)
        {
            ...
            if (infoSource != Generated.ed_informationsource.AdmAndraKund && infoSource != Generated.ed_informationsource.AdmSkapaKund)
            {
            ...
            if (conflict != null)
            {
                if (conflict.Contains(ContactEntity.Fields.cgi_socialsecuritynumber) && conflict.cgi_socialsecuritynumber.Equals(cgi_socialsecuritynumber))
                {
        ...
        public void CombineContacts(Plugin.LocalPluginContext localContext, IList<ContactEntity> conflicts)
        {
            ...
            foreach (ContactEntity c in conflicts)
            {
                MergeRequest merge = new MergeRequest()
                {
                    ...
                    UpdateContent = new ContactEntity()
                };
                //merge.SubordinateId = c.Id;
                //merge.Target = contact.ToEntityReference();
                //merge.PerformParentingChecks = false;

                //merge.UpdateContent = new ContactEntity
                //{
        ...
        public static ContactEntity FindOrCreateUnvalidatedContact(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(contact.EMailAddress1))
                {
                    contact.UpdateWithCustomerInfo(localContext, customerInfo);
        ...
        public static ContactEntity CreateUnvalidatedContact(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            if (customerInfo == null)
                return null;
        ...
        public static ContactEntity FindActiveContact(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(customerInfo.SocialSecurityNumber) && customerInfo.SwedishSocialSecurityNumberSpecified && customerInfo.SwedishSocialSecurityNumber)
            {
            ...
            if (contact == null && !string.IsNullOrWhiteSpace(customerInfo.FirstName) && !string.IsNullOrWhiteSpace(customerInfo.LastName) && !string.IsNullOrWhiteSpace(customerInfo.Email))
            {
                ...
                #endregion
                IList<ContactEntity> possibleMatches = XrmRetrieveHelper.RetrieveMultiple<ContactEntity>(localContext, query);
                ContactEntity email1Name = null, email1angeNamn = null, email2Name = null, email2angeNamn = null;
                if (possibleMatches.Count > 0)
                {
                    foreach (ContactEntity c in possibleMatches)
                    {
                        if (customerInfo.Email.Equals(c.EMailAddress1, StringComparison.InvariantCultureIgnoreCase) && customerInfo.FirstName.Equals(c.FirstName, StringComparison.InvariantCultureIgnoreCase) && customerInfo.LastName.Equals(c.LastName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            email1Name = c;
                        }
                        else if (customerInfo.Email.Equals(c.EMailAddress1, StringComparison.InvariantCultureIgnoreCase) && "Ange".Equals(c.FirstName, StringComparison.InvariantCultureIgnoreCase) && "Namn".Equals(c.LastName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            email1angeNamn = c;
                        }
                        else if (customerInfo.Email.Equals(c.EMailAddress2, StringComparison.InvariantCultureIgnoreCase) && customerInfo.FirstName.Equals(c.FirstName, StringComparison.InvariantCultureIgnoreCase) && customerInfo.LastName.Equals(c.LastName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            email2Name = c;
                        }
                        else if (customerInfo.Email.Equals(c.EMailAddress2, StringComparison.InvariantCultureIgnoreCase) && "Ange".Equals(c.FirstName, StringComparison.InvariantCultureIgnoreCase) && "Namn".Equals(c.LastName, StringComparison.InvariantCultureIgnoreCase))
                        {
                            email2angeNamn = c;
                        }
                    }
                    contact = email2angeNamn != null ? email2angeNamn : contact;
                    contact = email2Name != null ? email2Name : contact;
                    contact = email1angeNamn != null ? email1angeNamn : contact;
                    contact = email1Name != null ? email1Name : contact;
                }
            }
            return contact;
        }

        public void UpdateWithCustomerInfo(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            if (customerInfo == null)
                return;
            ...
            if (customerInfo.Source > 0)
                this.ed_InformationSource = (Generated.ed_informationsource)customerInfo.Source;

        }

        public static bool UpdateContactWithLead(ref ContactEntity contact, LeadEntity lead)
        {
            bool updated = false;
