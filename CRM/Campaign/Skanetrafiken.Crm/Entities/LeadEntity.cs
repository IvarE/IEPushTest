...
namespace Skanetrafiken.Crm.Entities
{
    public class LeadEntity : Generated.Lead
    {
        ...
        internal static IList<LeadEntity> FindExistingCampaignLeads(Plugin.LocalPluginContext localContext, LeadInfo leadInfo)
        {
            ...
            IList<LeadEntity> socSecLeads = null;
            ...
            List<LeadEntity> possibleMatches = XrmRetrieveHelper.RetrieveMultiple<LeadEntity>(localContext, query);
        ...
        public static void HandlePostLeadSetStateAsync(Plugin.LocalPluginContext localContext, EntityReference leadRef, OptionSetValue stateCode, OptionSetValue statusCode)
        {
            if (stateCode?.Value == (int)Generated.LeadState.Qualified)
            {
                ...
                if (campaign != null) {
                    ...
                    if (campaign.Contains(CampaignEntity.Fields.ed_ValidFromPhase1) && campaign.ed_ValidFromPhase1 != null &&
                        campaign.Contains(CampaignEntity.Fields.ed_ValidToPhase1) && campaign.ed_ValidToPhase1 != null)
                    {
                        IList<LeadEntity> connectedLeadsDR1 = XrmRetrieveHelper.RetrieveMultiple<LeadEntity>(localContext, new ColumnSet(false),
                            new FilterExpression
                            {
                                Conditions =
                                {
                                    ...
                                    new ConditionExpression(LeadEntity.Fields.ModifiedOn, ConditionOperator.OnOrBefore, campaign.ed_ValidToPhase1)
                                }
                            });
                    ...
                    if (campaign.Contains(CampaignEntity.Fields.ed_ValidFromPhase2) && campaign.ed_ValidFromPhase2 != null &&
                        campaign.Contains(CampaignEntity.Fields.ed_ValidToPhase2) && campaign.ed_ValidToPhase2 != null)
                    {
                        IList<LeadEntity> connectedLeadsDR2 = XrmRetrieveHelper.RetrieveMultiple<LeadEntity>(localContext, new ColumnSet(false),
                            new FilterExpression
                            {
                                Conditions =
                                {
                                    ...
                                    new ConditionExpression(LeadEntity.Fields.ModifiedOn, ConditionOperator.OnOrBefore, campaign.ed_ValidToPhase2)
                                }
                            });
        ...
        private void ThrowErrorIfConflictingCampaignLead(Plugin.LocalPluginContext localContext)
        {
            if (CampaignId != null && !Guid.Empty.Equals(CampaignId.Id))
            {
                ...
                FilterExpression nameEmailOrSocSecNrFilter = new FilterExpression(LogicalOperator.Or);
                // add socSecNr Filter
                if (!string.IsNullOrWhiteSpace(this.ed_Personnummer))
                {
                    nameEmailOrSocSecNrFilter.AddCondition(
                ...
                {
                    nameEmailOrSocSecNrFilter.AddFilter(new FilterExpression(LogicalOperator.And)
                    {
                        Conditions =
                        {
        ...
        public CustomerInfo ToLeadInfo(Plugin.LocalPluginContext localContext)
        {
            string[] products = { };
            ...
            return new LeadInfo()
            {
                ...
                CampaignDiscountPercent = campaignEntity.ed_DiscountPercent.HasValue ? (int)campaignEntity.ed_DiscountPercent : -1,
