...
namespace Endeavor.Crm.UnitTest
{
    ...
    public class SalesOrderFixture : PluginFixtureBase
    {
        ...
        #region Data for Kop och Skicka
        const int cOrderNumber_KopOchSkicka = 13131515;
        const int cOrderLineNumber_KopOchSkicka = 883623;
        ...
        /// <summary>
        /// Sales order for private customers not in used. 
        /// Category changed to Obsolete 190723, JoAn
        /// </summary>
        [Test, Category("Obsolete")]
        ...
        public void SalesOrderPostTest()
        {
            ...
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                CompanyRoleEntity role = XrmRetrieveHelper.RetrieveFirst<CompanyRoleEntity>(localContext, CompanyRoleEntity.EntityLogicalName, new ColumnSet(true));

                AccountEntity account = XrmRetrieveHelper.RetrieveFirst<AccountEntity>(localContext, new ColumnSet(AccountEntity.Fields.AccountNumber),
                        new FilterExpression()
                        {
                            Conditions =
                            {
                                 new ConditionExpression(AccountEntity.Fields.AccountId, ConditionOperator.Equal, role.ed_Account.Id)
                            }
                        });

                List<SalesOrderInfo> salesOrders = new List<SalesOrderInfo>();

                SalesOrderInfo correctSalesOrder = new SalesOrderInfo()
                {
                    InformationSource = 13,
                    OrderNo = "1212",
                    OrderTime = DateTime.Now,
                    PortalId = account.AccountNumber,
                    Customer = new CustomerInfo()
                    {
                        SocialSecurityNumber = role.ed_SocialSecurityNumber
                    },
                    SalesOrderLines = new[] {
                        new SalesOrderLineInfo()
                        {
                            PortalId = "111-unik",
                            Amount = 2,
                            Comment = "UnitTest",
                            Journey = "To the moon",
                            TicketId = "uniqueId",
                            TicketOfferTypeName = "test",
                            TicketOfferType = 1,
                            SalesOrderLineTravellers = new []
                            {
                                new SalesOrderLineTravellerInfo
                                {
                                    PortalId = "hello-unique",
                                    Count = 23,
                                    TravellerType = 1,
                                    TravellerTypeName = "test"
                                }
                            }
                        }
                    }
                ...
                salesOrders.Add(correctSalesOrder);

                // CREATE
                foreach (SalesOrderInfo salesOrder in salesOrders)
                {
                    try
                    {

                        var httpWebRequest = (HttpWebRequest)WebRequest.Create($"{WebApiTestHelper.WebApiRootEndpoint}SalesOrders/");
                        httpWebRequest.ContentType = "application/json";
                        httpWebRequest.Method = "POST";

                        using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                        {
                            string InputJSON = JsonConvert.SerializeObject(salesOrder, Newtonsoft.Json.Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                            streamWriter.Write(InputJSON);
                            streamWriter.Flush();
                            streamWriter.Close();
                        }
                        ...
                        using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                        {
                            // Result is 
                            var result = streamReader.ReadToEnd();
                            localContext.TracingService.Trace("Results={0}", result);
                        }
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("Error in Post SalesOrder: " + ex.Message, ex);
                    }
                }
        ...
        private Order_KopOchSkicka GenerateValidSalesOrderInfo_ForKopOchSkicka()
        {
            DateTime orderTime = DateTime.Now.ToUniversalTime();
            orderTime = orderTime.AddTicks(-(orderTime.Ticks % TimeSpan.TicksPerSecond));

            var orderLines = new List<OrderLine_KopOchSkicka>()
            {
                new OrderLine_KopOchSkicka() { CardNumber = "524242", OrderId = cOrderNumber_KopOchSkicka, OrderLineId = cOrderLineNumber_KopOchSkicka, Status = 1},
                new OrderLine_KopOchSkicka() { CardNumber = "8412331", OrderId = 31516, OrderLineId = 516882, Status = 1}
            };

            var order = new Order_KopOchSkicka()
            {
                OrderId = cOrderNumber_KopOchSkicka,
                Type = 17,
                Created = orderTime,
                ContactGuid = "04F42046-1700-EA11-80F0-005056B61FFF", //From Test-server
        ...
        /// </summary>
        [Test]
        public void TestAPI_KopOchSkickaSalesOrderPost()
        {
            // Connect to the Organization service. 
            // The using statement assures that the service proxy will be properly disposed.
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                CallApi($"{WebApiTestHelper.WebApiRootEndpoint}/SalesOrders", GenerateValidSalesOrderInfo_ForKopOchSkicka(),
                    "POST", typeof(SalesOrderInfo), delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                    {
        ...
        /// Call api
        /// </summary>
        /// <param name="endpoint">Api endpoint, adress to the controller.</param>
        /// <param name="body">Body content, the o</param>
        /// <param name="httpRequestMethod"></param>
        /// <param name="destination"></param>
        /// <param name="assert"></param>
        /// <param name="clearDataAction"></param>
