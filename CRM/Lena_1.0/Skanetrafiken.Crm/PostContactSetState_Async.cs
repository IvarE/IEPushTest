...
{
    public class PostContactSetState_Async : Plugin
    {
        /// <summary>
        /// </summary>
        //private readonly string preImageAlias = "preImage";

        public PostContactSetState_Async()
            : base(typeof(PostContactSetState_Async))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.SetState, ContactEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteContactSetStateAsync)));
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.SetStateDynamicEntity, ContactEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteContactSetStateAsync)));
        ...
        /// </remarks>
        protected void PostExecuteContactSetStateAsync(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["EntityMoniker"] is EntityReference)
            {
                ...
                try
                {
                    ContactEntity.HandlePostContactSetStateAsync(localContext, myEntityRef, state, status);
                }
                catch (Exception ex)
                {
