...
{
    public class PostCampaignUpdate_Async : Plugin
    {
        /// <summary>
        /// </summary>
        private readonly string preImageAlias = "preImage";

        public PostCampaignUpdate_Async()
            : base(typeof(PostCampaignUpdate_Async))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.Update, CampaignEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteCampaignUpdateAsync)));
        ...
        /// </remarks>
        protected void PostExecuteCampaignUpdateAsync(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                CampaignEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<CampaignEntity>();

                CampaignEntity preImage = Plugin.GetPreImage<CampaignEntity>(localContext, preImageAlias);
                ...
                try
                {
                    target.HandlePostCampaignUpdateAsync(localContext, preImage);
                }
                catch (Exception ex)
                {
