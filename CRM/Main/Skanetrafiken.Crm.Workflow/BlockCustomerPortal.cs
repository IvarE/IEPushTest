...
{
    public class BlockCustomerPortal : CodeActivity
    {
        [Input("SSN")]
        [RequiredArgument()]
        ...
        {//GENERATE CONTEXT
            ...
            try
            {
                ...
                string response = ExecuteCodeActivity(localContext, ssn, blocked);
                localContext.Trace($"BlockCustomerPortal finished. Responsemessage: {response}");
            }
            catch (Exception ex)
            {
        ...
        public static string ExecuteCodeActivity(Plugin.LocalPluginContext localContext, string ssn, bool blocked)
        {
            ...
            try
            {
                ...
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create($"{mklEndpoint}/api/block/users/{ssn}");
                ...
                {
                    // Result is 
                    InputJSON = "{\"socialSecurityNumber\": \"" + ssn + "\",  \"blocked\": " + blocked.ToString().ToLower() + "}";
