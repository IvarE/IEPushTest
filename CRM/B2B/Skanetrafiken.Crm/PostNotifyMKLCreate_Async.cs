...
{
    public class PostNotifyMKLCreate_Async : Plugin
    {
        public PostNotifyMKLCreate_Async()
            : base(typeof(PostNotifyMKLCreate_Async))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.Create, NotifyMKLEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteNotifyMKLCreate)));
        ...
        /// </remarks>
        protected void PostExecuteNotifyMKLCreate(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                NotifyMKLEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<NotifyMKLEntity>();
                
                try
                {
                    target.HandlePostNotifyMKLCreateAsync(localContext);
                }
                catch (Exception ex)
                {
