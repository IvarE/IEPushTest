...
{
    public class PreProductUpdate : Plugin
    {
        /// <summary>
        /// </summary>
        private readonly string preImageAlias = "preImage";

        public PreProductUpdate()
            : base(typeof(PreProductUpdate))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>(20, "Update", ProductEntity.EntityLogicalName, new Action<LocalPluginContext>(PreExecuteProductUpdate)));
        ...
        /// </remarks>
        protected void PreExecuteProductUpdate(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                ProductEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<ProductEntity>();

                ProductEntity preImage = Plugin.GetPreImage<ProductEntity>(localContext, preImageAlias);
                ...
                try
                {
                    target.HandlePreProductUpdate(localContext, preImage);
                }
                catch (Exception ex)
                {
