...
namespace Skanetrafiken.Crm.Entities
{
    public class CampaignEntity : Generated.Campaign
    {
        ...
        public void HandlePreCampaignUpdate(Plugin.LocalPluginContext localContext, CampaignEntity preImage)
        {
            ...
            {
                FilterExpression stageFilter = new FilterExpression(LogicalOperator.Or)
                {
                    Conditions =
                    {
                ...
                if (preImage.StageId != null)
                    stageFilter.AddCondition(new ConditionExpression(ProcessStageEntity.Fields.ProcessStageId, ConditionOperator.Equal, preImage.StageId));

                IList<ProcessStageEntity> stages = XrmRetrieveHelper.RetrieveMultiple<ProcessStageEntity>(localContext, new ColumnSet(ProcessStageEntity.Fields.StageCategory), stageFilter);
                ...
                if (targetStage != null || preImageStage != null)
                {
                    if (targetStage.StageCategory == Generated.processstage_category.Close)
                    {
                        ...
                        if ((preImageStage.StageCategory == Generated.processstage_category.Qualify && combined.ed_ValidToPhase1.HasValue && DateTime.Now.CompareTo(combined.ed_ValidToPhase1.Value.AddDays(1)) < 0) ||
                            (preImageStage.StageCategory == Generated.processstage_category.Develop && combined.ed_ValidToPhase2.HasValue && DateTime.Now.CompareTo(combined.ed_ValidToPhase2.Value.AddDays(1)) < 0))
                        {
                            throw new Exception("Campaign is not allowed to end before response time is complete");
                        }
                    }
                    else if (targetStage.StageCategory == Generated.processstage_category.Develop)
                    {
                        if (preImageStage.StageCategory == Generated.processstage_category.Qualify && combined.ed_ValidToPhase1.HasValue && DateTime.Now.CompareTo(combined.ed_ValidToPhase1.Value.AddDays(1)) < 0)
                        {
                            throw new Exception("Campaign is not allowed to enter stage 2 before response time of stage 1 is complete");
                        }
                    }
                }
            }
        ...
        {
            if (!(Contains(CampaignEntity.Fields.StateCode) && ed_ValidFromPhase1.HasValue && ed_ValidToPhase1.HasValue))
                return false;

            if (!Generated.CampaignState.Active.Equals(StateCode))
                return false;

            if (ed_ValidFromPhase2.HasValue && ed_ValidToPhase2.HasValue)
            {
                if ((DateTime.Now.CompareTo(ed_ValidFromPhase1) < 0 || DateTime.Now.CompareTo(ed_ValidToPhase1.Value.AddDays(1)) > 0) && (DateTime.Now.CompareTo(ed_ValidFromPhase2) < 0 || DateTime.Now.CompareTo(ed_ValidToPhase2.Value.AddDays(1)) > 0))
                    return false;
            }
            else
            {
                if (DateTime.Now.CompareTo(ed_ValidFromPhase1) < 0 || DateTime.Now.CompareTo(ed_ValidToPhase1.Value.AddDays(1)) > 0)
                    return false;
            }
        ...
        public void RecalculateStatisticsOnCampaign(Plugin.LocalPluginContext localContext)
        {
            int? totalLeadsBefore = this.ed_TotalLeads;
            int? qualifiedLeadsBefore = this.ed_QualifiedLeads;
            int? qualifiedLeadsDR1Before = this.ed_QualifiedLeadsDR1;
            int? qualifiedLeadsDR2Before = this.ed_QualifiedLeadsDR2;

            int qualifiedDR1new = this.QualifiedLeadsDR1Count(localContext);
            int qualifiedDR2new = this.QualifiedLeadsDR2Count(localContext);
            int qualifiedLeadsNew = qualifiedDR1new + qualifiedDR2new;
            int totalLeadsNew = this.TotalLeadsCount(localContext);

            if (this.ed_QualifiedLeadsDR1 != qualifiedDR1new)
                this.ed_QualifiedLeadsDR1 = qualifiedDR1new;

            if (this.ed_QualifiedLeadsDR2 != qualifiedDR2new)
                this.ed_QualifiedLeadsDR2 = qualifiedDR2new;

            if (this.ed_QualifiedLeads != qualifiedLeadsNew)
                this.ed_QualifiedLeads = qualifiedLeadsNew;

            if (this.ed_TotalLeads != totalLeadsNew)
                this.ed_TotalLeads = totalLeadsNew;

            XrmHelper.Update(localContext, this);
        }

        public int QualifiedLeadsDR1Count(Plugin.LocalPluginContext localContext)
        {
            var universalTime = this.ed_ValidFromPhase1.Value.ToUniversalTime();
            var localTime = this.ed_ValidToPhase1.Value.ToLocalTime();

            IList<CampaignResponseEntity> campaignResponsesDR1 = XrmRetrieveHelper.RetrieveMultiple<CampaignResponseEntity>(localContext, new ColumnSet(false),
                new FilterExpression
                {
                    Conditions =
                        {
                            new ConditionExpression(CampaignResponseEntity.Fields.CreatedOn, ConditionOperator.OnOrAfter, this.ed_ValidFromPhase1.Value.ToLocalTime()),
                            new ConditionExpression(CampaignResponseEntity.Fields.CreatedOn, ConditionOperator.OnOrBefore, this.ed_ValidToPhase1.Value.ToLocalTime().AddDays(1)),
                            new ConditionExpression(CampaignResponseEntity.Fields.RegardingObjectId, ConditionOperator.Equal, this.Id)
                        }
                });

            if (campaignResponsesDR1 != null)
                return campaignResponsesDR1.Count();
            else
                return 0;
        }

        public int QualifiedLeadsDR2Count(Plugin.LocalPluginContext localContext)
        {
            IList<CampaignResponseEntity> campaignResponsesDR2 = XrmRetrieveHelper.RetrieveMultiple<CampaignResponseEntity>(localContext, new ColumnSet(false),
                new FilterExpression
                {
                    Conditions =
                        {
                            new ConditionExpression(CampaignResponseEntity.Fields.CreatedOn, ConditionOperator.OnOrAfter, this.ed_ValidFromPhase2.Value.ToLocalTime()),
                            new ConditionExpression(CampaignResponseEntity.Fields.CreatedOn, ConditionOperator.OnOrBefore, this.ed_ValidToPhase2.Value.ToLocalTime().AddDays(1)),
                            new ConditionExpression(CampaignResponseEntity.Fields.RegardingObjectId, ConditionOperator.Equal, this.Id)
                        }
                });

            if (campaignResponsesDR2 != null)
                return campaignResponsesDR2.Count();
            else
                return 0;
        }

        public int TotalLeadsCount(Plugin.LocalPluginContext localContext)
        {
            IList<LeadEntity> totalLeads = XrmRetrieveHelper.RetrieveMultiple<LeadEntity>(localContext, new ColumnSet(false),
                new FilterExpression
                {
                    Conditions =
                        {
                            new ConditionExpression(LeadEntity.Fields.CampaignId, ConditionOperator.Equal, this.Id)
                        }
                });
