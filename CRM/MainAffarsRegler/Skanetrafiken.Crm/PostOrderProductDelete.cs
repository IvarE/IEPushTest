...
namespace Skanetrafiken.Crm
{
    public class PostOrderProductDelete : Plugin
    {
        ...
        public PostOrderProductDelete()
            : base(typeof(PostOrderProductDelete))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>(40, "Delete", OrderProductEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteOrderProductDelete)));
        ...
        /// </remarks>
        protected void PostExecuteOrderProductDelete(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is EntityReference)
            {
                ...
                try
                {
                    ...
                    if (preImage.SalesOrderId != null)
                    {
                        OrderEntity order = XrmRetrieveHelper.Retrieve<OrderEntity>(localContext, preImage.SalesOrderId, new ColumnSet(QuoteEntity.Fields.DiscountPercentage, QuoteEntity.Fields.DiscountAmount));

                        OrderProductEntity.UpdateSlotsCustomPrice(localContext, order);
                    }
