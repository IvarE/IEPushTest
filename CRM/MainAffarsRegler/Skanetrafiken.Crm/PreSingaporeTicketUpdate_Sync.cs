...
{
    public class PreSingaporeTicketUpdate_Sync : Plugin
    {
        string preImageAlias = "preImage";

        public PreSingaporeTicketUpdate_Sync()
     : base(typeof(PreSingaporeTicketUpdate_Sync))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PreOperation, Plugin.SdkMessageName.Update, SingaporeTicketEntity.EntityLogicalName, new Action<LocalPluginContext>(PreExecuteSingaporeTicketUpdateSync)));
        ...
        /// </remarks>
        protected void PreExecuteSingaporeTicketUpdateSync(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                SingaporeTicketEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<SingaporeTicketEntity>();
                ...
                try
                {
                    target.HandlePreSingaporeTicketUpdateSync(localContext, preImage);
                }
                catch (Exception ex)
                {
