...
namespace Skanetrafiken.Crm
{
    public class PostContactUpdate_Async : Plugin
    {
        ...
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.Update, ContactEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteContactUpdateAsync)));
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    ...
                    if(target.ed_BusinessContact != true)
                        target.ClearContactFieldsRelatedToSSN(localContext, postImage, preImage);
                }
                catch (Exception ex)
                {
