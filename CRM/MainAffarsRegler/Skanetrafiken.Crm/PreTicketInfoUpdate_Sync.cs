...
{
    public class PreTicketInfoUpdate_Sync : Plugin
    {
        string preImageAlias = "preImage";

        public PreTicketInfoUpdate_Sync()
            : base(typeof(PreTicketInfoUpdate_Sync))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PreOperation, Plugin.SdkMessageName.Update, TicketInfoEntity.EntityLogicalName, new Action<LocalPluginContext>(PreExecuteTicketInfoUpdateSync)));
        ...
        /// </remarks>
        protected void PreExecuteTicketInfoUpdateSync(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    target.HandlePreTicketInfoUpdateSync(localContext, preImage);
                }
                catch (Exception ex)
                {
