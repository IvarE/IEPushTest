...
{
    public class PreSlotCreate : Plugin
    {
        public PreSlotCreate()
            : base(typeof(PreSlotCreate))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PreOperation, Plugin.SdkMessageName.Create, SlotsEntity.EntityLogicalName, new Action<LocalPluginContext>(Execute)));
        ...
        protected void Execute(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    target.HandlePreSlotsEntityCreate(localContext);
                }
                catch (Exception ex)
                {
