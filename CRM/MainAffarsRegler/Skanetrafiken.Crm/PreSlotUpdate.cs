...
{
    public class PreSlotUpdate : Plugin
    {
        /// <summary>
        /// </summary>
        private readonly string preImageAlias = "preImage";

        public PreSlotUpdate()
            : base(typeof(PreSlotUpdate))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PreOperation,Plugin.SdkMessageName.Update , SlotsEntity.EntityLogicalName, new Action<LocalPluginContext>(PreExecute)));
        ...
        /// </remarks>
        protected void PreExecute(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    target.HandlePreSlotsEntityUpdate(localContext, preImage);
                }
                catch (Exception ex)
                {
