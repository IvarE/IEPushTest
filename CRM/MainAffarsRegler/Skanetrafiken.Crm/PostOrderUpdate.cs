...
namespace Skanetrafiken.Crm
{
    public class PostOrderUpdate : Plugin
    {
        ...
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>((int)Plugin.SdkMessageProcessingStepStage.PostOperation, Plugin.SdkMessageName.Update, OrderEntity.EntityLogicalName, new Action<LocalPluginContext>(PostExecuteOrderUpdate)));
        ...
        /// </remarks>
        protected void PostExecuteOrderUpdate(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                try
                {
                    ...
                    OrderEntity target = ((Entity)localContext.PluginExecutionContext.InputParameters["Target"]).ToEntity<OrderEntity>();

                    OrderEntity preImage = Plugin.GetPreImage<OrderEntity>(localContext, preImageAlias);

                    if (preImage == null)
                        throw new InvalidPluginExecutionException("Pre-Image not registered correctly.");

                    OrderEntity.HandleOrderEntityUpdate(localContext, target, preImage);
                }
                catch (Exception ex)
                {
