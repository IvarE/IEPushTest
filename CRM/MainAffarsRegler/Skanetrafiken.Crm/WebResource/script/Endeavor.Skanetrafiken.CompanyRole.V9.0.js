...
if (typeof (Endeavor.Skanetrafiken.CompanyRole) == "undefined") {
    Endeavor.Skanetrafiken.CompanyRole = {
        ...
        headLoad: function (successCallback) {
            ...
            if (typeof Sdk == "undefined" || typeof Sdk.ed_BlockCompanyRolePortalRequest == "undefined") {
                jsUrl = globalContext.getClientUrl() + "/WebResources/ed_/script/Sdk.ed_BlockCompanyRolePortal.min.js";
                jsUrls.push(jsUrl);
            }
        ...
        onBlockCompanyRoleShow: function (executionContext) {
            debugger;
        ...
        onUnblockCompanyRoleShow: function (executionContext) {
            debugger;
            var formContext = executionContext.getFormContext();
        ...
        blockCompanyRolePortal: function (executionContext) {
            ...
            try {
                Endeavor.Skanetrafiken.CompanyRole.headLoad(function () { Endeavor.Skanetrafiken.CompanyRole.blockCompanyRolePortalSuccessCallback(formContext); });
            } catch (e) {
                Endeavor.Skanetrafiken.CompanyRole.alertCustomDialog(e.message);
            }
        },
        ...
        blockCompanyRolePortalSuccessCallback: function (formContext) {
            ...
            if (formContext.getAttribute("ed_socialsecuritynumber")) {
                var SSN = formContext.getAttribute("ed_socialsecuritynumber").getValue();
            }
            ...
            if (account && account.getValue() && account.getValue()[0] && account.getValue()[0].id) {
                ...
                //TODO WebAPI
                var url = Endeavor.Common.Data.getOrganizationServiceEndpoint() + "AccountSet?$select=AccountNumber&$filter=AccountId eq(guid'" + guid + "')";
                var result = Endeavor.Common.Data.fetchJSONResults(url);

                PortalID = result.entities[0].AccountNumber;
            }
            ...
            if (blocked === true) {
                try {
                    var request = new Sdk.ed_BlockCompanyRolePortalRequest(SSN, PortalID, false);
                    var response = Sdk.Sync.execute(request);
                    Endeavor.Skanetrafiken.CompanyRole.alertCustomDialog("företagsroll avblockerad!");
                }
                catch (e) {
                    Endeavor.Skanetrafiken.CompanyRole.alertCustomDialog("Kunde inte avblockera företagsroll. Var god försök igen senare.");
                }
            }
            else {
                try {
                    var request = new Sdk.ed_BlockCompanyRolePortalRequest(SSN, PortalID, true);
                    var response = Sdk.Sync.execute(request);
                    Endeavor.Skanetrafiken.CompanyRole.alertCustomDialog("företagsroll spärrad.");
                }
                catch (e) {
                    Endeavor.Skanetrafiken.CompanyRole.alertCustomDialog("Kunde inte spärra företagsroll. Var god försök igen senare.");
                }
            }
        }
    };
