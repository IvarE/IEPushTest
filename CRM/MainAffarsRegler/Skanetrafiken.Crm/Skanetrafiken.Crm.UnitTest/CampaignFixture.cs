...
namespace Endeavor.Crm.UnitTest
{
    ...
    public class CampaignFixture : PluginFixtureBase
    {
        ...
        public void TestActualCampaignEndRules()
        {
            ...
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                try
                {
                    ...
                    {
                        if (!"Campaign is not allowed to enter stage 2 before response time of stage 1 is complete".Equals(e.Message))
                            throw;
                    }
                    ...
                    {
                        Id = campaign.Id,
                        ed_ValidFromPhase1 = DateTime.Now.AddDays(-4),
                        ed_ValidToPhase1 = DateTime.Now.AddDays(-1),
                        StageId = new Guid(stage3DR2),
                        ProcessId = new Guid(processId)
                    };
                    XrmHelper.Update(localContext.OrganizationService, updateEnt);
        ...
        [Test, Category("Debug")]
        public void DebugCampaignPlugin()
        {
            // Connect to the Organization service. 
            // The using statement assures that the service proxy will be properly disposed.
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                CampaignEntity preImage = XrmRetrieveHelper.RetrieveFirst<CampaignEntity>(
                    localContext,
                    new QueryExpression()
                    {
                        ...
                        Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(CampaignEntity.Fields.CodeName, ConditionOperator.Equal, "CMP-01020-K0H2")
                            }
                        }
                    });

                CampaignEntity target = new CampaignEntity
                {
                    Id = preImage.Id,
                    StageId = new Guid("ceeb25ac-c704-4612-a296-61cef8986c70"),
                    ProcessId = new Guid("2d95bbf8-e9ff-4c4b-a988-53e4cc1225a5")
                };
