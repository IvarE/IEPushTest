...
namespace Endeavor.Crm
{
    ...
    public abstract class Plugin : IPlugin
    {
        ...
        public const string cAsUserSystemSuffix = "asusersystem";
        ...
        public class LocalPluginContext
        {
            ...
            public LocalPluginContext(IServiceProvider serviceProvider, bool executeAsUserSystem = false)
            {
                ...
                // Use the factory to generate the Organization Service.
                if (executeAsUserSystem)
                {
                    executeAsUserId = Guid.Empty;
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            bool runInContextOfUserSystemFlag = false;

            foreach (var registeredEvent in registeredEvents)
            {
                if (registeredEvent.Item4 != null && registeredEvent.Item4.Method != null && !String.IsNullOrWhiteSpace(registeredEvent.Item4.Method.Name))
                {
                    if (registeredEvent.Item4.Method.Name.ToLower().EndsWith(cAsUserSystemSuffix))
                    {
                        runInContextOfUserSystemFlag = true;
                        break;
                    }
                }
            }

            LocalPluginContext localcontext = new LocalPluginContext(serviceProvider, runInContextOfUserSystemFlag);

            if (runInContextOfUserSystemFlag)
            {
                localcontext.Trace("Executing in context of built-in user SYSTEM.");
            }
