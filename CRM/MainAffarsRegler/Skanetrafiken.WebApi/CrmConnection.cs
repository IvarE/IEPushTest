...
using Properties = Skanetrafiken.Crm.Properties;
...
namespace Skanetrafiken.Crm
{
    ...
    public class CrmConnection
    {
        ...
        public static string GetCrmConnectionString(string credentialFilePath)
        {
            ...
            //Configuration config1 = ConfigurationManager.OpenExeConfiguration(@"C:\Stuff\Skånetrafiken\Main\Skanetrafiken.WebApi\bin\Skanetrafiken.WebApi.dll");
            //foreach (ConnectionStringSettings connectionStringSettings in config1.ConnectionStrings.ConnectionStrings)
            //{
            //    // Remove any connection string not applicable to CRM.
            //    if (ValidConnectionString(connectionStringSettings.ConnectionString))
            //    {
            ...
            {
                throw new Exception(string.Format(CultureInfo.InvariantCulture, Properties.Resources.ConnectionStringMissing));
            }
            ...
            // TO DO: To solve the issue with the encryption
            SetCrmConnectionPassword(credentialFilePath, ref connectionString);
        ...
        /// </summary>
        private static void SetCrmConnectionPassword(string credentialFilePath, ref string connectionString)
        {
            ...
            {
//                SaveCredentials(credentialFilePath, password);
                throw new Exception(string.Format(CultureInfo.InvariantCulture, Properties.Resources.IllegalConnectionStringPassword, ExternalPasswordPlaceHolder));
            }
            ...
            connectionString = connectionString.Insert(passwordStartAt, LoadCredentials(credentialFilePath));
        }

        /// <summary>
        /// Store password in the credential file.
        /// </summary>
        internal static void SaveCredentials(string filePath, string password)
        {
            XDocument saveDoc = new XDocument(
                new XElement("Root",
                        new XElement("Password", EncryptString(ToSecureString(password)))
                )
            );
        ...
        /// </summary>
        internal static string LoadCredentials(string filePath)
        {
            if (!File.Exists(filePath))
            {
                throw new FileNotFoundException(Properties.Resources.CredentialsFileNotFound, filePath);
            }
            XDocument doc = XDocument.Load(filePath);
            
            //var encrypted = EncryptString( ToSecureString(""));
