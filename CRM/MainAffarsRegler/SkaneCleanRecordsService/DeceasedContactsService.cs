...
{
    public partial class DeceasedContactsService : ServiceBase
    {
        ...
        internal static byte[] Entropy = System.Text.Encoding.Unicode.GetBytes("DeceasedContactsService");

        internal static string CredentialFilePath
        {
            get
            {
                return Environment.ExpandEnvironmentVariables(Properties.Settings.Default.CredentialsFilePathDeceasedContacts);
            }
        }
        ...
        public static void InitializeScheduler(IScheduler scheduler)
        {
            ...
            _log.Info($"Scheduling Inactivate Deceased Contacts Job");

            IJobDetail scheduleUploadJob = JobBuilder.Create<InactivateDeceasedContacts>()
            .WithIdentity(InactivateDeceasedContacts.JobName, InactivateDeceasedContacts.GroupName)
            .UsingJobData(jobDataMap)
            .WithDescription(InactivateDeceasedContacts.JobDescription)
            .Build();

            ITrigger scheduleUploadTrigger = TriggerBuilder.Create()
                  .WithIdentity(InactivateDeceasedContacts.TriggerName, InactivateDeceasedContacts.GroupName)
                  .WithCronSchedule(Properties.Settings.Default.InactivateContactsScheduleCronExpression)
                  .WithDescription(InactivateDeceasedContacts.TriggerDescription)
        ...
        public DeceasedContactsService()
        {
            InitializeComponent();
        }

        public DeceasedContactsService(IContainer container)
        {
            container.Add(this);
