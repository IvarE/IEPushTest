...
{
    public class PreQualifyLead : Plugin
    {
        /// <summary>
        /// </summary>
        private readonly string preImageAlias = "preImage";

        public PreQualifyLead()
            : base(typeof(PreQualifyLead))
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>(
                ...
                new Action<LocalPluginContext>(PreExecuteQualifyLead)));
        ...
        /// </remarks>
        protected void PreExecuteQualifyLead(LocalPluginContext localContext)
        {
            ...
            localContext.TracingService.Trace("Found entityId:{0}, name:{1}", leadid.Id, leadid.Name);

            try
            {
                LeadEntity.HandlePreLeadSetState(localContext, leadid, new OptionSetValue((int)Generated.LeadState.Qualified), new OptionSetValue((int)Generated.lead_statuscode.Qualified));
            }
            catch (Exception ex)
            {
