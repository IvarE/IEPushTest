...
namespace Skanetrafiken.Crm
{
    public class PreContactCreate : Plugin
    {
        ...
        {
            base.RegisteredEvents.Add(new Tuple<int, string, string, Action<LocalPluginContext>>(20, "Create", ContactEntity.EntityLogicalName, new Action<LocalPluginContext>(PreExecuteContactCreate)));
        ...
        protected void PreExecuteContactCreate(LocalPluginContext localContext)
        {
                ...
                localContext.PluginExecutionContext.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    target.HandlePreContactCreate(localContext);
                }
                catch (Exception ex)
                {
