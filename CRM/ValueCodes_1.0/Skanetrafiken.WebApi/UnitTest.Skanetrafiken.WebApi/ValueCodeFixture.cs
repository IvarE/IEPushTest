...
namespace Endeavor.Crm.UnitTest
{
    ...
    public class ValueCodeFixture : PluginFixtureBase
    {
        ...
        public void CallApi(string endpoint, object body, Type destination, Action<Plugin.LocalPluginContext, string, HttpWebResponse> assert = null, Action<Plugin.LocalPluginContext> clearDataAction = null, string httpMethod = "POST")
        {
            ...
            ContactEntity contact = null, int? typeOfValueCode = null, string firstName = null, string lastName = null)
        {
            ...
            return new ValueCodeLossCreation()
            {
                ContactId = contact?.Id,
                TravelCard = travelCard,
                deliveryMethod = deliveryMethod.HasValue == true ? deliveryMethod.Value : 0,
                Email = email,
                Mobile = mobile,
                FirstName = firstName,
                LastName = lastName,
                TypeOfValueCode = typeOfValueCode.HasValue == true ? typeOfValueCode.Value : 0
            };
        }
        ...
        #region All tests towards Value Code type 5

        public string c_codeId = "5151251";

        private ValueCodeEvent CreateTestData_ValueCodeEvent(DateTime created, decimal amount = 0, int voucherType = 1, long? eanCode = null,
            int? couponId = null, DateTime? redeemed = null, string vouchercode = null)
        {
            var valueCode = new ValueCodeEvent()
            {
                amount = amount,
                created = created,
                tag = "Tag_test",
        ...
        public void Test_Create_ValueCodeType5()
        {
            CallApi($"{WebApiTestHelper.WebApiRootEndpoint_LocalTest}/ValueCode",
                CreateTestData_ValueCodeEvent(new DateTime(2020, 01, 01), 0, 5, 1315151, 41215, DateTime.UtcNow, c_codeId_TransactionTest), typeof(ValueCodeEvent),
                delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                {
                    DeleteVK5_TestData(l, c_codeId);
                    var query = new QueryExpression()
                    {
                        EntityName = ValueCodeEntity.EntityLogicalName,
                        ColumnSet = new ColumnSet(true),
                        Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(ValueCodeEntity.Fields.ed_CodeId, ConditionOperator.Equal, c_codeId)
                            }
                        }
                    };

                    var valueCode = XrmRetrieveHelper.RetrieveFirst<ValueCodeEntity>(l, query);
                    Assert.NotNull(valueCode);
                    Assert.AreEqual(c_codeId, valueCode.ed_CodeId);
                    Assert.AreEqual(new Money(0), valueCode.ed_Amount);
                    Assert.AreEqual(5, (int)valueCode.ed_ValueCodeTypeGlobal);
                    Assert.AreEqual("1315151", valueCode.ed_Ean);
                    Assert.AreEqual((int)Generated.ed_ValueCodeState.Inactive, (int)valueCode.statecode);
                    Assert.AreEqual((int)Generated.ed_valuecode_statuscode.Inlost, (int)valueCode.statuscode);
                    Assert.AreEqual(1000, valueCode.ed_OriginalAmount);

                    DeleteVK5_TestData(l, valueCode.ed_CodeId);
                });
        }
        ...
        private void DeleteVK5_TestData(Plugin.LocalPluginContext localContext, string codeId)
        {
            ...
            var query = new QueryExpression()
            {
                ...
                Criteria =
                {
                    Conditions =
                    {
                        new ConditionExpression(ValueCodeEntity.Fields.ed_CodeId, ConditionOperator.Equal, codeId)
                    }
                }
            };

            var f_valueCode = XrmRetrieveHelper.RetrieveFirst<ValueCodeEntity>(localContext, query);
            if (f_valueCode != null)
                XrmHelper.Delete(localContext, f_valueCode.ToEntityReference());
        }

        #endregion

        #region All tests towards Value Code Transactions

        public string c_codeId_TransactionTest = "5231241";


        [Test]
        public void Fullflow_Local_Test_CreateTransactionData()
        {
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                // This statement is required to enable early-bound type support.
                ...
                DeleteTransactionFullFlowTestData(localContext);


                #region Scenario 1 - Förseningsersättning - 1 Transaction and inactivate VK
                var newVK = CreateTestData_ValueCode_For_Transaction(localContext, 100, Generated.ed_valuecodetypeglobal.Forseningsersattning);
                var valueCodeEvent_data = CreateTestData_ValueCodeEvent(DateTime.UtcNow.AddHours(1), 0, 1, 4141, 315613, null, c_codeId_TransactionTest); //Inte inlöst

                CallApi($"{WebApiTestHelper.WebApiRootEndpoint_LocalTest}/ValueCode", valueCodeEvent_data, typeof(ValueCodeEvent),
                delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                {

                    var query = new QueryExpression()
                    {
                        EntityName = ValueCodeTransactionEntity.EntityLogicalName,
                        ColumnSet = new ColumnSet(true),
                        Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(ValueCodeTransactionEntity.Fields.statecode, ConditionOperator.Equal, (int)Generated.ed_valuecodetransactionState.Active),
                                new ConditionExpression(ValueCodeTransactionEntity.Fields.ed_ValueCode, ConditionOperator.Equal, newVK.Id)
                            }
                        }
                    };

                    var trans = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query);
                    Assert.AreEqual(trans.Count, 1);
                    Assert.AreEqual(newVK.Id, trans[0].ed_ValueCode.Id);
                    Assert.AreNotEqual(newVK.ed_Amount.Value, trans[0].ed_Balance);
                    Assert.Less(trans[0].ed_Balance, newVK.ed_Amount.Value);
                    Assert.AreEqual(newVK.ed_Amount.Value, trans[0].ed_PreviousBalance);
                ...
                #region Scenario 2 - Förseningsersättning - 2 Transactions and inactivate VK
                newVK = CreateTestData_ValueCode_For_Transaction(localContext, 100, Generated.ed_valuecodetypeglobal.InlostReskassa);
                valueCodeEvent_data = CreateTestData_ValueCodeEvent(DateTime.UtcNow.AddHours(1), 20, 1, 4141, 315613, null, c_codeId_TransactionTest); //Inte inlöst
                ...
                valueCodeEvent_data = CreateTestData_ValueCodeEvent(DateTime.UtcNow.AddHours(1), 0, 1, 4141, 315613, null, c_codeId_TransactionTest); //Inlöst

                var updatedVK = XrmRetrieveHelper.Retrieve<ValueCodeEntity>(localContext, newVK.ToEntityReference(), new ColumnSet(true));
                ...
                delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                {
                    ...
                    var trans = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query).OrderByDescending(w => w.ed_Balance).ToList();
                    Assert.AreEqual(trans.Count, 2);
                    ...
                    //Seond Trans
                    Assert.AreEqual(updatedVK.Id, trans[1].ed_ValueCode.Id);
                    Assert.AreNotEqual(updatedVK.ed_Amount.Value, trans[1].ed_Balance);
                    Assert.Less(trans[1].ed_Balance, updatedVK.ed_Amount.Value);
                    Assert.AreEqual(updatedVK.ed_Amount.Value, trans[1].ed_PreviousBalance);
                ...
                #region Scenario 3 - Value Code Type 5 (Presentkort) - Create VK through VK5 flow and VK is still active

                //Update value code
                valueCodeEvent_data = CreateTestData_ValueCodeEvent(DateTime.UtcNow.AddHours(1), 20, 5, 4141, 315613, null, c_codeId_TransactionTest);
                ...
                delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                {
                    var query = new QueryExpression()
                    {
                        ...
                        Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(ValueCodeEntity.Fields.ed_CodeId, ConditionOperator.Equal, c_codeId_TransactionTest),
                                new ConditionExpression(ValueCodeEntity.Fields.statecode, ConditionOperator.Equal, (int)Generated.ed_ValueCodeState.Active)
                            }
                        }
                    };

                    var newValueCode = XrmRetrieveHelper.RetrieveFirst<ValueCodeEntity>(localContext, query);
                    Assert.NotNull(newValueCode);
                    Assert.AreEqual(newValueCode.ed_CodeId, c_codeId_TransactionTest);
                    Assert.AreEqual(newValueCode.ed_Amount.Value, 20);
                    ...
                    query = new QueryExpression()
                    {
                        ...
                        Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(ValueCodeTransactionEntity.Fields.ed_ValueCode, ConditionOperator.Equal, newValueCode.Id)
                            }
                        }
                    };
                    ...
                    var trans = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query);
                    Assert.AreEqual(trans.Count, 1);
                    Assert.AreEqual(newValueCode.Id, trans[0].ed_ValueCode.Id);
                    Assert.AreEqual(newValueCode.ed_Amount.Value, trans[0].ed_Balance);
                    Assert.AreEqual(newValueCode.ed_Amount.Value, trans[0].ed_PreviousBalance);
                });
                ...
                #region Scenario 4 - VK 5 (Presentkort) - Create new vk through VK5 flow and 2 Transactions & inactivate VK
                ...
                var query1 = new QueryExpression()
                {
                    EntityName = ValueCodeEntity.EntityLogicalName,
                    ColumnSet = new ColumnSet(true),
                    Criteria =
                ...
                var newValueCode_1 = XrmRetrieveHelper.RetrieveFirst<ValueCodeEntity>(localContext, query1);
                Assert.NotNull(newValueCode_1);
                Assert.AreEqual(newValueCode_1.ed_CodeId, c_codeId_TransactionTest);
                Assert.AreEqual(newValueCode_1.ed_Amount.Value, 20);

                query1 = new QueryExpression()
                {
                    EntityName = ValueCodeTransactionEntity.EntityLogicalName,
                    ColumnSet = new ColumnSet(true),
                    Criteria =
                        {
                            Conditions =
                            {
                                new ConditionExpression(ValueCodeTransactionEntity.Fields.ed_ValueCode, ConditionOperator.Equal, newValueCode_1.Id)
                            }
                        }
                };

                var trans_1 = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query1);
                Assert.AreEqual(trans_1.Count, 1);
                Assert.AreEqual(newValueCode_1.Id, trans_1[0].ed_ValueCode.Id);
                Assert.AreEqual(newValueCode_1.ed_Amount.Value, trans_1[0].ed_Balance);
                Assert.AreEqual(newValueCode_1.ed_Amount.Value, trans_1[0].ed_PreviousBalance);

                //ValueCode and Transaction has been created. Now send an update.

                valueCodeEvent_data = CreateTestData_ValueCodeEvent(DateTime.UtcNow.AddHours(1), 0, 5, 4141, 315613, null, c_codeId_TransactionTest);
                ...
                delegate (Plugin.LocalPluginContext l, string s, HttpWebResponse h)
                {
                    ...
                    var trans1 = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query).OrderByDescending(w => w.ed_Balance).ToList();

                    //Seond Trans
                    Assert.AreEqual(newValueCode_1.Id, trans1[1].ed_ValueCode.Id);
                    Assert.AreNotEqual(newValueCode_1.ed_Amount.Value, trans1[1].ed_Balance);
                    Assert.Less(trans1[1].ed_Balance, newValueCode_1.ed_Amount.Value);
                    Assert.AreEqual(newValueCode_1.ed_Amount.Value, trans1[1].ed_PreviousBalance);
        ...
        private ValueCodeEntity CreateTestData_ValueCode_For_Transaction(Plugin.LocalPluginContext localContext, decimal amount, Generated.ed_valuecodetypeglobal ed_Valuecodetypeglobal)
        {
            var newVK = new ValueCodeEntity()
            {
                ed_CodeId = c_codeId_TransactionTest,
                statuscode = Generated.ed_valuecode_statuscode.Active,
                ed_Amount = new Money(amount),
                ed_CreatedTimestamp = DateTime.UtcNow.AddHours(1),
                ed_ValueCodeTypeGlobal = ed_Valuecodetypeglobal
            };

            newVK.Id = XrmHelper.Create(localContext, newVK);
            var updateVK = new ValueCodeEntity()
            {
                Id = newVK.Id,
                ed_name = newVK.Id.ToString()
            };

            XrmHelper.Update(localContext, updateVK);

            return newVK;
        }

        private void DeleteTransactionFullFlowTestData(Plugin.LocalPluginContext localContext)
        {
            ...
            if(valueCode != null)
            {
                ...
                query = new QueryExpression()
                {
                    ...
                    Criteria =
                    {
                        Conditions =
                        {
                            //new ConditionExpression(ValueCodeTransactionEntity.Fields.ed_ValueCode, ConditionOperator.Equal, valueCode.ToEntityReference())
                        }
                    }
                };

                var transactions = XrmRetrieveHelper.RetrieveMultiple<ValueCodeTransactionEntity>(localContext, query);
                foreach (var transaction in transactions)
                {
                    XrmHelper.Delete(localContext, transaction.ToEntityReference());
                }
        ...
        public void TemplateTest()
        {
            using (_serviceProxy = ServerConnection.GetOrganizationProxy(Config))
            {
                ...
                ValueCodeHandler.GetValueCodeTemplate(localContext, null, 7);
            }
        }
