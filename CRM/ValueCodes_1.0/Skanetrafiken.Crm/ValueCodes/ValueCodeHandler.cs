...
namespace Skanetrafiken.Crm.ValueCodes
{
    public static class ValueCodeHandler
    {
        ...
        {
            localContext.TracingService.Trace($"--> Entering {nameof(GetValueCodeTemplate)}");
            if (localContext == null)
                throw new Exception($"LocalContext is empty. Check your connection.");
            ...
            if (templateNumber == 7) //Försening (Saldo)  
            {
                localContext.TracingService.Trace($"Changing templateNumber '{templateNumber}' to 1");
                templateNumber = 1;
            }
            else if (templateNumber == 6) //Ersättning (Saldo)
            {
                localContext.TracingService.Trace($"Changing templateNumber '{templateNumber}' to 4");
                templateNumber = 4;
            }
            ...
            localContext.TracingService.Trace($"<-- Exiting {nameof(GetValueCodeTemplate)}");
            return template;
        }
            ...
            string clientsPhoneNumber, string clientsEmailAddress, TravelCardEntity travelCard, RefundEntity refund)
        {
            ...
            // 1 = förseningsersätting eller förseningsersätting (Saldo)
            if (voucherType == 1 || voucherType == 7)
            {
            ...
            else if (voucherType == 3)
            {
                ...
                //if (travelCardInfo?.cgi_ValidTo < DateTime.Now && periodPrice > 0)
                //{
                //    periodPrice = 0;
                //}
            ...
            // 4 = ersättning
            else if (voucherType == 4 || voucherType == 6)
            {
                RefundEntity refundInfo = XrmRetrieveHelper.Retrieve<RefundEntity>(localContext, refund.Id, new ColumnSet(
                    RefundEntity.Fields.cgi_Caseid));
            ...
            ValueCodeApprovalEntity valueCodeApproval, int voucherType, TravelCardEntity travelCard, float amount, int totalAmountSentToVoucherService, decimal originalPeriodPrice, float originalReskassa)
        {
            ...
            if (response != null)
            {
                ...
                if (voucherType == (int)Generated.ed_valuecodetypeglobal.ErsattningsarendeSaldo)
                    valueCode.ed_ValueCodeTypeGlobal = Generated.ed_valuecodetypeglobal.ErsattningsarendeSaldo;
                ...
                if (voucherType == (int)Generated.ed_valuecodetypeglobal.ForseningsersattningSaldo)
                    valueCode.ed_ValueCodeTypeGlobal = Generated.ed_valuecodetypeglobal.ForseningsersattningSaldo;
