    ...
    public class ContactEntity : Generated.Contact
    {
        ...
        public CustomerInfo ToCustomerInfo(Plugin.LocalPluginContext localContext)
        {
            return new CustomerInfo()
            {
                ...
                ) ? null : new CustomerInfoSpecAddressBlock()
                {
        ...
        public void HandlePreContactUpdate(Plugin.LocalPluginContext localContext, ContactEntity preImage)
        {
            ...
            CustomerInfo info = combined.ToCustomerInfo(localContext);
            ...
            if (ed_InformationSource == Generated.ed_informationsource.BytEpost && ed_EmailToBeVerified == CustomerUtility._NEWEMAILDONE)
        ...
        public static ContactEntity FindOrCreateUnvalidatedContact(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(contact.EMailAddress1))
                {
                    ContactEntity updateContact = contact.UpdateWithCustomerInfo(localContext, customerInfo);
        ...
        public static ContactEntity FindActiveContact(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            ...
            // Not found on personnr?
            // Search for name and e-mail.
            // ****************************
            if (contact == null && !string.IsNullOrWhiteSpace(customerInfo.FirstName) && !string.IsNullOrWhiteSpace(customerInfo.LastName) && !string.IsNullOrWhiteSpace(customerInfo.Email))
            {
                ...
                // Update away "Ange Namn" if present
                bool update = false;
                if (contact != null)
                {
        ...
        private ContactEntity UpdateWithCustomerInfo(Plugin.LocalPluginContext localContext, CustomerInfo customerInfo)
        {
            ContactEntity updateContact = new ContactEntity
            {
                ContactId = this.ContactId,
            ...
            if (!string.IsNullOrWhiteSpace(customerInfo.Mobile) && !customerInfo.Mobile.Equals(this.Telephone2))
            {
                this.Telephone2 = customerInfo.Mobile;
                updateContact.Telephone2 = customerInfo.Mobile;
                updated = true;
